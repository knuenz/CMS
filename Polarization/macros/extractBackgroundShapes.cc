...
int main(int argc, char** argv) {
  ...
  for(int yBin = 0; yBin < jpsi::kNbRapForPTBins-1; ++yBin) {
      ...
      std::stringstream binName,cutString, cutStringMC;
      ...
      cutString << "JpsiPt > " << jpsi::pTRange[yBin+1][ptBin] << " && JpsiPt < " << jpsi::pTRange[yBin+1][ptBin+1] << " && "
		...
		<< "(JpsiMass < " << jpsi::polMassJpsi[yBin+1]-jpsi::sigmaMassJpsi[yBin+1]*jpsi::nSigBkgLow 
		<< " || JpsiMass > " << jpsi::polMassJpsi[yBin+1]+jpsi::sigmaMassJpsi[yBin+1]*jpsi::nSigBkgHigh << ")";
