...
def buildDataAndCategories(ws,options,args):
    ...
    if options.gen is not None:
        gennum = int(options.gen)
        print gennum
    ...
    jPsiVprob     = RooRealVar('JpsiVprob','',0.01,1)
    jPsiHXcosth   = None
    jPsiHXphi     = None

    if options.gen is not None:
        generation    = RooRealVar('generation','generation',gennum-0.5,gennum+0.5)
    if options.onlyPROMPTdata is not None:
        MCType    = RooRealVar('MCType','MCType',-0.5,+0.5)
    if options.onlyNONPROMPTdata is not None:
        MCType    = RooRealVar('MCType','MCType',0.5,1.5)
...
#    dataVars.add(jPsiVprob)
    ...
    for rap_bin in range(1,len(jpsi.pTRange)):
        ...
        for pt_bin in range(len(jpsi.pTRange[rap_bin])):
            ...
            #cutStringGEN = '('+generation.GetName()+' > '+'2.5'+' && '+generation.GetName()+' < '+'3.5'+')'
            
            cutString   = cutStringPt+' && '+cutStringY#+' && '+cutStringGEN #+' && '+cutStringMT
...
def buildMassAndLifetimePDF(ws):
    ...
    ws.factory('RooExponential::bkgMassShape(JpsiMass,bkgLambda[1])')
    #ws.factory('RooChebychev::bkgMassShape(JpsiMass,{p0[1],p1[1],p2[1]})')    
    #lifetime
    ...
    ws.factory('RooDecay::backgroundSSDRawL(Jpsict,bkgTauSSDL[1],promptLifetimeRaw,RooDecay::SingleSided)')
    #ws.factory('RooDecay::backgroundFDRaw(Jpsict,bkgTauFD[.3,1],promptLifetimeRaw,RooDecay::Flipped)')
    ws.factory('RooDecay::backgroundDSDRawL(Jpsict,bkgTauDSDL[1],promptLifetimeRaw,RooDecay::DoubleSided)')    
    ws.factory('SUM::backgroundRawL(fBkgSSDL[1]*backgroundSSDRawL,backgroundDSDRawL)')
    ...
    ws.factory('SUM::backgroundRawR(fBkgSSDR[1]*backgroundSSDRawR,backgroundDSDRawR)')
    ...
    ws.factory('RooExtendPdf::promptExtMass(promptRawMass,sum::nPrompt(nPromptSignal[1000,0,1000000]))')
    ws.factory('RooExtendPdf::nonpromptExtMass(nonpromptRawMass,sum::nNonPrompt(nNonPromptSignal[500,0,1000000]))')
    ws.factory('RooExtendPdf::backgroundExtMass(backgroundRawMass,sum::nBackground(nBackgroundSignal[0],nBackgroundL[0],nBackgroundR[0]))')
...
if __name__ == '__main__':
    ...
    parser.add_option('--DoFit',dest='do_fit',default=False,action='store_true',help='Do not fit ML')
    parser.add_option('--onlyPROMPTdata',dest='onlyPROMPTdata',help='=1 -> use only PROMPT sample')
    parser.add_option('--onlyNONPROMPTdata',dest='onlyNONPROMPTdata',help='=1 -> use only NON PROMPT sample')
