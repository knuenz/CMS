    ...
    for rap_bin in rap_bins:
        ...
        for pt_bin in pt_bins:
            ...
            if options.scenario is not None:
                scennum = int(options.scenario)
                print scennum
           
            
            resfile=ROOT.TFile('RooFitResult_'+options.JOBNAME+'_scen'+str(scennum)+'_rap'+str(rap_bin)+'_pt'+str(pt_bin+1)+'.root','UPDATE');

            migradfitresult = minuit.save('migrad_pol_fitresult_rap'+str(rap_bin)+'_pt'+str(pt_bin+1)+'_'+options.fitFrame+'_gen'+options.gen)
            getattr(ws,'import')(migradfitresult)           
            
            resfile.Add(migradfitresult);
            
            minuit.hesse()

            hessefitresult = minuit.save('hesse_pol_fitresult_rap'+str(rap_bin)+'_pt'+str(pt_bin+1)+'_'+options.fitFrame+'_gen'+options.gen)
            getattr(ws,'import')(hessefitresult)           

            hessefitresult.Print()
                       
            resfile.Add(hessefitresult);
            resfile.Write()   
            resfile.Close()
            ...
            #minosfitresult = minuit.save('minos_pol_fitresult_rap'+str(rap_bin)+'_pt'+str(pt_bin+1))
            #getattr(ws,'import')(minosfitresult)           

            #minosfitresult.Print()
...
def buildPolarizationPDF(ws,options):
    ...
    ws.factory('lambda_theta_'+options.fitFrame+'_p[0,-2,2]')
    
    ws.factory('lambda_thetaphi_'+options.fitFrame+'_p[0,-2,2]')
    ws.var('lambda_thetaphi_'+options.fitFrame+'_p').setMin(-sqrt(2)/2)
    ws.var('lambda_thetaphi_'+options.fitFrame+'_p').setMax(sqrt(2)/2)    

    ws.factory('lambda_theta_'+options.fitFrame+'_np[0,-2,2]')    
    ws.factory('lambda_thetaphi_'+options.fitFrame+'_np[0,-2,2]')
    ws.var('lambda_thetaphi_'+options.fitFrame+'_np').setMin(-sqrt(2)/2)
    ws.var('lambda_thetaphi_'+options.fitFrame+'_np').setMax(sqrt(2)/2)

    if options.lambdaPhiSub == 'lambda_phi':
        ws.factory('lambda_phi_'+options.fitFrame+'_p[0,-2,2]')
        ws.factory('lambda_phi_'+options.fitFrame+'_np[0,-2,2]')
    if options.lambdaPhiSub == 'F_invar':
    ...
    if options.lambdaPhiSub == 'lambda_tilde':
        ws.factory('lambda_tilde_p[0,-2,100]')
        ws.factory('lambda_tilde_np[0,-2,100]')
...
#            getattr(ws,'import')(bkgInNP,ROOT.RooFit.RecycleConflictNodes())
            ...
            if options.fixBfrac:
                ...
                #cutStringCtau = '('+jPsicTau.GetName()+' > 0.1)'
                #data2 = ws.data('data_rap'+str(rap_bin)+'_pt'+str(pt_bin+1))
                #bfracVars = RooArgSet(jPsiMass,jPsicTau)
                ...
                #bfracData2 = bfracData.reduce(ROOT.RooFit.SelectVars(bfracVars),
                #                         ROOT.RooFit.Cut(cutStringCtau),
                #                         ROOT.RooFit.Name('data_for_normalizing_bfrac2'),
                #                         ROOT.RooFit.Title('data_for_normalizing_bfrac2'))
                #                                                              
                #bfrac = jpsi.Bfrac[rap_bin-1][pt_bin]
                #numEvents_in_Signal_regions = bfracData.numEntries()         
                #numEvents_in_nonPrompt_signal_region = bfracData2.numEntries()
                #
                #print numEvents_in_Signal_regions
                #print numEvents_in_nonPrompt_signal_region
                #
                #nPinNP__ = 0
                #nNPinNP__ = 0.99
                #
                #absolut_nNPinP = (numEvents_in_Signal_regions*bfrac-numEvents_in_nonPrompt_signal_region*nNPinNP__)
                #absolut_nPinP = (numEvents_in_Signal_regions*(1-bfrac))-(1-nNPinNP__)*numEvents_in_nonPrompt_signal_region
                #
                ##checkN = absolut_nNPinP+absolut_nPinP+numEvents_in_nonPrompt_signal_region-numEvents_in_Signal_regions
                ##print checkN
                #
                #nNPinP__ = absolut_nNPinP/(absolut_nNPinP+absolut_nPinP)
                #nPinP__ = (1-nNPinP__)
                #
                #ws.factory('nPinP_[1]')
                #ws.factory('nPinNP_[1]')
                #ws.factory('nNPinNP_[1]')
                #ws.factory('nNPinP_[1]')
				#           
                #ws.var('nPinP_').setVal(nPinP__)
                #ws.var('nPinNP_').setVal(nPinNP__)
                #ws.var('nNPinNP_').setVal(nNPinNP__)
                #ws.var('nNPinP_').setVal(nNPinP__)
                #
                #fPinP_actual__=ws.var('nPinP_').getVal()
                #fPinNP_actual__=ws.var('nPinNP_').getVal()
                #fNPinP_actual__=ws.var('nNPinP_').getVal()
                #fNPinNP_actual__=ws.var('nNPinNP_').getVal()
                #

                fPinP_actual = RooRealVar('fPinP_actual','fPinP_actual',jpsi.fPinP_actual[rap_bin-1][pt_bin])
                
                fPinNP_actual = RooRealVar('fPinNP_actual','fPinNP_actual',0)
                
                fNPinP_actual = RooRealVar('fNPinP_actual','fNPinP_actual',jpsi.fNPinP_actual[rap_bin-1][pt_bin])
                
                fNPinNP_actual = RooRealVar('fNPinNP_actual','fNPinNP_actual',jpsi.fNPinNP_actual[rap_bin-1][pt_bin])

                
                #fPinP_actual.setVal(jpsi.fPinP_actual[rap_bin-1][pt_bin])
                #fPinNP_actual.setVal(0)
                #fNPinP_actual.setVal(jpsi.fNPinP_actual[rap_bin-1][pt_bin])
                #fNPinNP_actual.setVal(jpsi.fNPinNP_actual[rap_bin-1][pt_bin])
                
                fPinP_actual.setConstant()
                fPinNP_actual.setConstant()
                fNPinP_actual.setConstant()
                fNPinNP_actual.setConstant()            
                
                print 'new fix'
                print fPinP_actual.getVal()
                print fPinNP_actual.getVal()
                print fNPinNP_actual.getVal()
                print fNPinP_actual.getVal()
            ...
            alphaPinP = 0#PinP*((PinP*(1-PinP))/errPinP/errPinP - 1) 
            betaPinP = 0#(1-PinP)*((PinP*(1-PinP))/errPinP/errPinP - 1) + 1
            
            errNPinP = 0#( (1/(nPinP+nNPinP+nBGinP) + nNPinP/pow(nPinP+nNPinP+nBGinP,2))*nNPinPerr +
                         #nNPinP/pow(nPinP+nNPinP+nBGinP,2)*nPinPerr + nNPinP/pow(nPinP+nNPinP+nBGinP,2)*nBGinPerr )

            alphaNPinP = 0#NPinP*((NPinP*(1-NPinP))/errNPinP/errNPinP - 1) 
            betaNPinP = 0#(1-NPinP)*((NPinP*(1-NPinP))/errNPinP/errNPinP - 1) + 1
            
            errBGinP = 0#( (1/(nPinP+nNPinP+nBGinP) + nBGinP/pow(nPinP+nNPinP+nBGinP,2))*nBGinPerr +
                         #nBGinP/pow(nPinP+nNPinP+nBGinP,2)*nPinPerr + nBGinP/pow(nPinP+nNPinP+nBGinP,2)*nNPinPerr )
            
            alphaBGinP = 0#BGinP*((BGinP*(1-BGinP))/errBGinP/errBGinP - 1) 
            betaBGinP = 0#(1-BGinP)*((BGinP*(1-BGinP))/errBGinP/errBGinP - 1) + 1
            
            errPinNP  = 0#( (1/(nPinNP+nNPinNP+nBGinNP) + nPinNP/pow(nPinNP+nNPinNP+nBGinNP,2))*nPinNPerr +
                         # nPinNP/pow(nPinNP+nNPinNP+nBGinNP,2)*nNPinNPerr + nPinNP/pow(nPinNP+nNPinNP+nBGinNP,2)*nBGinNPerr )
            
            alphaPinNP = 0#PinNP*((PinNP*(1-PinNP))/errPinNP/errPinNP - 1) 
            betaPinNP = 0#(1-PinNP)*((PinNP*(1-PinNP))/errPinNP/errPinNP - 1) + 1
            
            errNPinNP = 0#( (1/(nPinNP+nNPinNP+nBGinNP) + nNPinNP/pow(nPinNP+nNPinNP+nBGinNP,2))*nNPinNPerr +
                          #nNPinNP/pow(nPinNP+nNPinNP+nBGinNP,2)*nPinNPerr + nNPinNP/pow(nPinNP+nNPinNP+nBGinNP,2)*nBGinNPerr )
            
            alphaNPinNP = 0#NPinNP*((NPinNP*(1-NPinNP))/errNPinNP/errNPinNP - 1) 
            betaNPinNP = 0#(1-NPinNP)*((NPinNP*(1-NPinNP))/errNPinNP/errNPinNP - 1) + 1
            
            errBGinNP = 0#( (1/(nPinNP+nNPinNP+nBGinNP) + nBGinNP/pow(nPinNP+nNPinNP+nBGinNP,2))*nBGinNPerr +
                          #nBGinNP/pow(nPinNP+nNPinNP+nBGinNP,2)*nNPinNPerr + nBGinNP/pow(nPinNP+nNPinNP+nBGinNP,2)*nPinNPerr )
        
            alphaBGinNP = 0#BGinNP*((BGinNP*(1-BGinNP))/errBGinNP/errBGinNP - 1)
            betaBGinNP = 0#(1-BGinNP)*((BGinNP*(1-BGinNP))/errBGinNP/errBGinNP - 1) + 1
            ...
            nonPromptPOLArgList = RooArgList(ws.pdf('polPdf'+options.fitFrame+'NonPrompt_'+str(rap_bin)+'_'+str(pt_bin+1)))
            nonPromptPOLFracList = RooArgList(ws.var('fNPinNP_actual_rap'+str(rap_bin)+'_pt'+str(pt_bin+1)))


            if options.doPrompt:
                print 'doPrompt'
                nonPromptPOLArgList.add(ws.pdf('polPdf'+options.fitFrame+'Prompt_'+str(rap_bin)+'_'+str(pt_bin+1)))
                nonPromptPOLFracList.add(ws.var('fPinNP_actual_rap'+str(rap_bin)+'_pt'+str(pt_bin+1)))                         
            if options.doNonPrompt:
                print 'doNonPrompt'
...
if __name__ == '__main__':
    ...
    parser.add_option('--BfracTruth',dest='fixBfrac',default=False,action='store_true',help='fix Bfraction to MC Truth')
    parser.add_option('--scenario',dest='scenario',default=100,help='set scenario')
    parser.add_option('--gen',dest='gen',help='use respective generated sample')
    parser.add_option('--JOBNAME',dest='JOBNAME',default='NOjobnameSpecified',help='JOBNAME')
