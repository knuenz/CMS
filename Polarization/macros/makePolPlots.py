...
from ROOT import TFile, TTree, TChain, RooGExpModel, RooAddition, RooMinuit, TVectorD, TStyle
...
def main(options,args):
    ...
    ROOT.gROOT.ProcessLine('.L ColorContours.cc+')
    ROOT.gSystem.mkdir('Plots')
    ROOT.gSystem.mkdir('Plots/NLL')
    
    #setup integration
    intConf = ROOT.RooAbsReal.defaultIntegratorConfig()
    intConf.setEpsAbs(1e-5)
    intConf.setEpsRel(1e-5)
    #intConf.Print('v')
...
################################ NLL PLOTS ##################################
                    ...
                    if options.nll:
                        ...
                        ROOT.ColorContours(104)
                        #ROOT.ColorCoutours(55)
                        
                        valLam_thPR=ws.var('lambda_theta_'+fitFrame+'_p').getVal()
                        valLam_tilPR=ws.var('lambda_tilde_p').getVal()
                        valLam_thphPR=ws.var('lambda_thetaphi_'+fitFrame+'_p').getVal()
                        valLam_thNP=ws.var('lambda_theta_'+fitFrame+'_np').getVal()
                        valLam_tilNP=ws.var('lambda_tilde_np').getVal()
                        valLam_thphNP=ws.var('lambda_thetaphi_'+fitFrame+'_np').getVal()
                        valLam_nNP=ws.var('nNonPromptPol').getVal()
                        valLam_nPR=ws.var('nPromptPol').getVal()
                        
                        range_nll2=1
                        for j in range(range_nll2):
                            range_nll=4
                            for i in range(range_nll):
                                print i
                                if i == 0:
                                    Var1=ws.var('lambda_theta_'+fitFrame+'_p')
                                    Var2=ws.var('lambda_tilde_p')
                                    Var1.SetTitle('#lambda^{'+fitFrame+'}_{#theta PR}')
                                    Var2.SetTitle('#tilde{#lambda}^{'+fitFrame+'}_{PR}')
                                    valVar1=valLam_thPR
                                    valVar2=valLam_tilPR
                                if i == 1:
                                    Var1=ws.var('lambda_theta_'+fitFrame+'_p')
                                    Var2=ws.var('lambda_theta_'+fitFrame+'_np')
                                    Var1.SetTitle('#lambda^{'+fitFrame+'}_{#theta PR}')
                                    Var2.SetTitle('#lambda^{'+fitFrame+'}_{#theta NP}')
                                    valVar1=valLam_thPR
                                    valVar2=valLam_thNP

                                if i == 2:
                                    Var1=ws.var('lambda_theta_'+fitFrame+'_np')
                                    Var2=ws.var('lambda_tilde_np')
                                    Var1.SetTitle('#lambda^{'+fitFrame+'}_{#theta NP}')
                                    Var2.SetTitle('#tilde{#lambda}^{'+fitFrame+'}_{NP}') 
                                    valVar1=valLam_thNP
                                    valVar2=valLam_tilNP

                                if i == 3:
                                    Var1=ws.var('lambda_tilde_p')
                                    Var2=ws.var('lambda_tilde_np')
                                    Var1.SetTitle('#tilde{#lambda}^{'+fitFrame+'}_{PR}')
                                    Var2.SetTitle('#tilde{#lambda}^{'+fitFrame+'}_{NP}') 
                                    valVar1=valLam_tilPR
                                    valVar2=valLam_tilNP
                                
                                ws.var('lambda_theta_'+fitFrame+'_p').setVal(valLam_thPR)
                                ws.var('lambda_tilde_p').setVal(valLam_tilPR)
                                ws.var('lambda_thetaphi_'+fitFrame+'_p').setVal(valLam_thphPR)
                                ws.var('lambda_theta_'+fitFrame+'_np').setVal(valLam_thNP)
                                ws.var('lambda_tilde_np').setVal(valLam_tilNP)
                                ws.var('lambda_thetaphi_'+fitFrame+'_np').setVal(valLam_thphNP)
                                ws.var('nNonPromptPol').setVal(valLam_nNP)
                                ws.var('nPromptPol').setVal(valLam_nPR)                                
                                
                                
                                nllReal=ws.pdf('PPdf'+fitFrame+'_'+str(rap_bin)+'_'+str(pt_bin+1)).createNLL(dsig,
                                ...
                                nllfunc=nllReal.asTF(RooArgList(Var1,Var2))
                                if j==0:
                                    nllfunc.SetRange(valVar1-0.1,valVar2-0.1,valVar1+0.1,valVar2+0.1)
                                    plotnameapp = 'close'
                                if j==1:
                                    plotnameapp = 'total'
                                    if i==0:
                                        nllfunc.SetRange(-1,-1,1,1)
                                    if i==1:
                                        nllfunc.SetRange(-1,-1,1,1)
                                    if i==2:
                                        nllfunc.SetRange(-1,-1,1,1)
                                    if i==3:
                                        nllfunc.SetRange(-1,-1,1,1)
                                nllfunc.SetNpx(50)
                                nllfunc.GetHistogram().SetXTitle(str(Var1.getTitle()))
                                nllfunc.GetHistogram().SetYTitle(str(Var2.getTitle()))
                                #nllfunc.GetHistogram().SetTitleOffset(1,'X')
                                nllfunc.GetHistogram().SetTitleOffset(1.5,'Y')
                                
                                ROOT.gStyle.SetPadRightMargin(0.25)
                                ROOT.gStyle.SetPadLeftMargin(0.15)
                                #ROOT.gStyle.SetPadBottomMargin(0.1)
                                nll_canvas = ROOT.TCanvas('nll_canvas_'+Var1.GetName()+'_'+Var2.GetName()+'_'+str(plotnameapp),'nll_canvas'+str(i),1600,1200)
                                nll_canvas.SetFillColor(ROOT.kWhite) 
                                ...
                                #pm3d1 = ROOT.TMarker(0.5,0.5,1);
                                #pm3d1.SetPoint(1,0.5,0.7)
                                #pm3d1.SetMarkerSize(10)
                                #pm3d1.SetMarkerColor(4)
                                #pm3d1.SetMarkerStyle(2)
                                #pm3d1.Draw()
                                                                
                                plotdir = '/afs/hephy.at/scratch/k/knuenz/CMSSW_3_8_1/src/JPsiPolarizationSave3/Plots/LogLikelihood/SMOOTH/scenario3/' 
                                #plotdir = '/afs/hephy.at/user/k/knuenz/www/Polarization/MCclosureTest/NLLplots/SMOOTH/scenario3/'
                                nll_canvas.SaveAs(str(plotdir)+'NLL_'+fitFrame+'_rap'+str(rap_bin)+'_pT'+str(pt_bin+1)+'_'+Var1.GetName()+'_'+Var2.GetName()+'_'+str(plotnameapp)+'.png')
                                nll_canvas.SaveAs(str(plotdir)+'NLL_'+fitFrame+'_rap'+str(rap_bin)+'_pT'+str(pt_bin+1)+'_'+Var1.GetName()+'_'+Var2.GetName()+'_'+str(plotnameapp)+'.root')
                                nll_canvas.Write()
...
#############################################################################
                    ...
                    ws.pdf('PPdf'+fitFrame+'_'+str(rap_bin)+
                           '_'+str(pt_bin+1)).plotOn(phiFramePr,
                                                     #RooFit.VisualizeError(ws.genobj('hesse_pol_fitresult_rap'+str(rap_bin)+'_pt'+str(pt_bin+1)+'_'+fitFrame+'_gen'+options.gen),3,True),
                    ...
                    ws.var('lambda_theta_'+fitFrame+'_p').setVal(0.5)
                    ws.var('lambda_tilde_p').setVal(0.5)
                    ws.var('lambda_thetaphi_'+fitFrame+'_p').setVal(0.0)
                    ws.var('lambda_theta_'+fitFrame+'_np').setVal(-0.5)
                    ws.var('lambda_tilde_np').setVal(-0.5)
                    ws.var('lambda_thetaphi_'+fitFrame+'_np').setVal(0.0)                    
                    
                    ws.pdf('PPdf'+fitFrame+'_'+str(rap_bin)+
                          '_'+str(pt_bin+1)).plotOn(phiFramePr,RooFit.LineColor(ROOT.kGreen),
                                                    RooFit.ProjWData(RooArgSet(ws.cat('mlRegion')),dpr))
                    
                    ws.pdf('PPdf'+fitFrame+'_'+str(rap_bin)+
                          '_'+str(pt_bin+1)).plotOn(costhFramePr,RooFit.LineColor(ROOT.kGreen),
                                                    RooFit.ProjWData(RooArgSet(ws.cat('mlRegion')),dpr))
                    
                    ws.pdf('PPdf'+fitFrame+'_'+str(rap_bin)+
                          '_'+str(pt_bin+1)).plotOn(phiFrameNP,RooFit.LineColor(ROOT.kGreen),
                                                    RooFit.ProjWData(RooArgSet(ws.cat('mlRegion')),dnp))

                    ws.pdf('PPdf'+fitFrame+'_'+str(rap_bin)+
                          '_'+str(pt_bin+1)).plotOn(costhFrameNP,RooFit.LineColor(ROOT.kGreen),
                                                    RooFit.ProjWData(RooArgSet(ws.cat('mlRegion')),dnp))
                    ...
                    canvasS=ROOT.TCanvas('canvasSs','canvasSs',3200,2600)
                    canvasS.Divide(2,2)
                    canvasS.SetFillColor(ROOT.kWhite) 
                    ROOT.gPad.SetFillColor(ROOT.kWhite)
                    canvasS.cd(1)
                    ROOT.gPad.SetFillColor(ROOT.kWhite)
                    phiFramePr.Draw();                                   
                    ...
                    plotdir = '/afs/hephy.at/scratch/k/knuenz/CMSSW_3_8_1/src/JPsiPolarizationSave3/Plots/Polarization/GENlevel/' 
                    #plotdir = '/afs/hephy.at/user/k/knuenz/www/Polarization/MCclosureTest/NLLplots/SMOOTH/scenario3/'
                    ROOT.gSystem.mkdir(str(plotdir))
                    canvasS.SaveAs(str(plotdir)+'1DProjection_'+fitFrame+'_rap'+str(rap_bin)+'_pT'+str(pt_bin+1)+'.png')
                    canvasS.SaveAs(str(plotdir)+'1DProjection_'+fitFrame+'_rap'+str(rap_bin)+'_pT'+str(pt_bin+1)+'.root')
                    canvasS.Write()
...
if __name__ == '__main__':
    ...
    parser.add_option('--nll',dest='nll',default=False,action='store_true',help='Plot Nll')
