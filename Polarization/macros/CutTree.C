...
//	Long64_t jentry=NMin;
    ...
    TNtuple* pseudodata = new TNtuple("data","data","JpsiMass:JpsiPt:JpsiRap:Jpsict:costh_CS:phi_CS:costh_HX:phi_HX:JpsiType_idx:MCType_idx:JpsictErr");
    TNtuple* data = new TNtuple("data","data","JpsiMass:JpsiPt:JpsiRap:Jpsict:costh_CS:phi_CS:costh_HX:phi_HX:JpsiType_idx:MCType_idx:JpsictErr");

 //   TNtuple* data = new TNtuple("data","data","JpsictErr:Jpsict");
    ...
    for (Long64_t jentry=0; jentry<NMin;jentry++) {
       	...
       	pseudodata->Fill(JpsiMass,JpsiPt,JpsiRap,Jpsict,costh_CS,phi_CS,costh_HX,phi_HX,JpsiType_idx,MCType_idx,JpsictErr);

        }

    for (Long64_t jentry=NMin; jentry<nentries;jentry++) {
   		    	 if(jentry % 1000 == 0) printf("event %d\n", (Int_t) jentry);
...
// 	  fprintf(outputFile, "%f\n",Jpsict);
   	...
   	data->Fill(JpsiMass,JpsiPt,JpsiRap,Jpsict,costh_CS,phi_CS,costh_HX,phi_HX,JpsiType_idx,MCType_idx,JpsictErr);

    }

    pseudodata->Print();

    pseudodata->SaveAs("/scratch/knuenz/Polarization/RootInput/TNtuple_red_pseudo_LambdaBerr.root");

	data->Print();

	data->SaveAs("/scratch/knuenz/Polarization/RootInput/MCPromptCtauErr.root");
