...
int main(int argc, char** argv) {
  ...
  TFile *output = new TFile("jPsiFit_Bins_PR_FSR_NPTry.root","UPDATE");
...
//  data = new RooDataSet("data","Concatentated MC Samples",samples,varlist);//,"MCweight");

  Char_t *fileNameIn = "/scratch/knuenz/Polarization/RootInput/TTree_NPerr.root";
  TFile* fIn = new TFile(fileNameIn);
  TTree* dataTrees = (TTree*)fIn->Get("data");
  data = new RooDataSet("data","Supplied Data Non Prompt",varlist,Import(*dataTrees),WeightVar(MCweight));



  for(int yBin = 0; yBin < jpsi::kNbRapForPTBins-1; ++yBin) {
	...
	theModel->initModel(JpsiMass,Jpsict,costh_CS,phi_CS);
      ...
      theModel->Print();
      if(pereverr){
	RooFitResult* NPResult = theModel->getLifetimeModel()->nonPrompt()->fitTo(*thisBinNonPrompt,RooFit::NumCPU(2),RooFit::Save(true),RooFit::Timer(true),
