...
void PlotRapPt(int ptBinMin, int ptBinMax, int rapBin, double yMin, double yMax, char yAxisTitle[200], char filename[200], double lmean[ToyMC::nPtBins], double lmeanerr_low[ToyMC::nPtBins], double lmeanerr_high[ToyMC::nPtBins], bool DrawLine, double lamline, char GraphFolder[200], char GraphName[200], bool SaveGraph, int nState, int nFrame){
	...
	plotHisto = plotCanvas->DrawFrame(onia::pTRange[rapBin][ptBinMin-1],yMin,onia::pTRange[rapBin][ptBinMax],yMax);
	...
	TLine* RefLine = new TLine( onia::pTRange[rapBin][ptBinMin-1], yLine, onia::pTRange[rapBin][ptBinMax], yLine );
	...
	for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
		ptCentre_[pt]=ToyMC::ptCentre[rapBin-1][ptBin-1];
		ptCentreErr_low[pt]=TMath::Abs(ToyMC::ptCentre[rapBin-1][ptBin-1]-onia::pTRange[rapBin][ptBin-1]);
		ptCentreErr_high[pt]=TMath::Abs(ToyMC::ptCentre[rapBin-1][ptBin-1]-onia::pTRange[rapBin][ptBin]);
	pt++;
	}
	...
	sprintf(outfilename,"%s/TGraphResults_%dSUps_temp.root",GraphFolder,nState);cout<<outfilename<<endl;
...
void PlotComparisonRapPt(int ptBinMin, int ptBinMax, int rapBin, double yMin, double yMax, char yAxisTitle[200], char filename[200], double lmean1[ToyMC::nPtBins], double lmean1err_low[ToyMC::nPtBins], double lmean1err_high[ToyMC::nPtBins], double lmean2[ToyMC::nPtBins], double lmean2err_low[ToyMC::nPtBins], double lmean2err_high[ToyMC::nPtBins], double lmean3[ToyMC::nPtBins], double lmean3err_low[ToyMC::nPtBins], double lmean3err_high[ToyMC::nPtBins], double lamline, int nState){
	...
	int pt=0;
	for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
		ptCentre_[pt]=ToyMC::ptCentre[rapBin-1][ptBin-1]+ToyMC::ptCentre[rapBin-1][ptBin-1]*.02;
		ptCentreErr_low[pt]=TMath::Abs(ToyMC::ptCentre[rapBin-1][ptBin-1]-onia::pTRange[rapBin][ptBin-1]);
		ptCentreErr_high[pt]=TMath::Abs(ToyMC::ptCentre[rapBin-1][ptBin-1]-onia::pTRange[rapBin][ptBin]);
	pt++;
	}
...
int main(int argc, char** argv) {
	...
	cout<<"";
	  ...
	  double param_lth_CS_mean[NrapBins][NptBins],	param_lth_CS_meanerr[NrapBins][NptBins],			param_lth_CS_sigma[NrapBins][NptBins],		param_lth_CS_sigma_high[NrapBins][NptBins],		param_lth_CS_sigma_low[NrapBins][NptBins],		param_lth_CS_sigmaerr[NrapBins][NptBins];
	  double param_lph_CS_mean[NrapBins][NptBins],	param_lph_CS_meanerr[NrapBins][NptBins],			param_lph_CS_sigma[NrapBins][NptBins],		param_lph_CS_sigma_high[NrapBins][NptBins],		param_lph_CS_sigma_low[NrapBins][NptBins],		param_lph_CS_sigmaerr[NrapBins][NptBins];
	  double param_ltp_CS_mean[NrapBins][NptBins],	param_ltp_CS_meanerr[NrapBins][NptBins],			param_ltp_CS_sigma[NrapBins][NptBins],		param_ltp_CS_sigma_high[NrapBins][NptBins],		param_ltp_CS_sigma_low[NrapBins][NptBins],		param_ltp_CS_sigmaerr[NrapBins][NptBins];
	  double param_lthstar_CS_mean[NrapBins][NptBins],	param_lthstar_CS_meanerr[NrapBins][NptBins],	param_lthstar_CS_sigma[NrapBins][NptBins],	param_lthstar_CS_sigma_high[NrapBins][NptBins],	param_lthstar_CS_sigma_low[NrapBins][NptBins],	param_lthstar_CS_sigmaerr[NrapBins][NptBins];
	  double param_lphstar_CS_mean[NrapBins][NptBins],	param_lphstar_CS_meanerr[NrapBins][NptBins],	param_lphstar_CS_sigma[NrapBins][NptBins],	param_lphstar_CS_sigma_high[NrapBins][NptBins],	param_lphstar_CS_sigma_low[NrapBins][NptBins],	param_lphstar_CS_sigmaerr[NrapBins][NptBins];
	  double param_ltilde_CS_mean[NrapBins][NptBins],	param_ltilde_CS_meanerr[NrapBins][NptBins],		param_ltilde_CS_sigma[NrapBins][NptBins],	param_ltilde_CS_sigma_high[NrapBins][NptBins],	param_ltilde_CS_sigma_low[NrapBins][NptBins],	param_ltilde_CS_sigmaerr[NrapBins][NptBins];

	  double param_lth_HX_mean[NrapBins][NptBins],	param_lth_HX_meanerr[NrapBins][NptBins],			param_lth_HX_sigma[NrapBins][NptBins],		param_lth_HX_sigma_high[NrapBins][NptBins],		param_lth_HX_sigma_low[NrapBins][NptBins],		param_lth_HX_sigmaerr[NrapBins][NptBins];
	  double param_lph_HX_mean[NrapBins][NptBins],	param_lph_HX_meanerr[NrapBins][NptBins],			param_lph_HX_sigma[NrapBins][NptBins],		param_lph_HX_sigma_high[NrapBins][NptBins],		param_lph_HX_sigma_low[NrapBins][NptBins],		param_lph_HX_sigmaerr[NrapBins][NptBins];
	  double param_ltp_HX_mean[NrapBins][NptBins],	param_ltp_HX_meanerr[NrapBins][NptBins],			param_ltp_HX_sigma[NrapBins][NptBins],		param_ltp_HX_sigma_high[NrapBins][NptBins],		param_ltp_HX_sigma_low[NrapBins][NptBins],		param_ltp_HX_sigmaerr[NrapBins][NptBins];
	  double param_lthstar_HX_mean[NrapBins][NptBins],	param_lthstar_HX_meanerr[NrapBins][NptBins],	param_lthstar_HX_sigma[NrapBins][NptBins],	param_lthstar_HX_sigma_high[NrapBins][NptBins],	param_lthstar_HX_sigma_low[NrapBins][NptBins],	param_lthstar_HX_sigmaerr[NrapBins][NptBins];
	  double param_lphstar_HX_mean[NrapBins][NptBins],	param_lphstar_HX_meanerr[NrapBins][NptBins],	param_lphstar_HX_sigma[NrapBins][NptBins],	param_lphstar_HX_sigma_high[NrapBins][NptBins],	param_lphstar_HX_sigma_low[NrapBins][NptBins],	param_lphstar_HX_sigmaerr[NrapBins][NptBins];
	  double param_ltilde_HX_mean[NrapBins][NptBins],	param_ltilde_HX_meanerr[NrapBins][NptBins],		param_ltilde_HX_sigma[NrapBins][NptBins],	param_ltilde_HX_sigma_high[NrapBins][NptBins],	param_ltilde_HX_sigma_low[NrapBins][NptBins],	param_ltilde_HX_sigmaerr[NrapBins][NptBins];

	  double param_lth_PX_mean[NrapBins][NptBins],	param_lth_PX_meanerr[NrapBins][NptBins],			param_lth_PX_sigma[NrapBins][NptBins],		param_lth_PX_sigma_high[NrapBins][NptBins],		param_lth_PX_sigma_low[NrapBins][NptBins],		param_lth_PX_sigmaerr[NrapBins][NptBins];
	  double param_lph_PX_mean[NrapBins][NptBins],	param_lph_PX_meanerr[NrapBins][NptBins],			param_lph_PX_sigma[NrapBins][NptBins],		param_lph_PX_sigma_high[NrapBins][NptBins],		param_lph_PX_sigma_low[NrapBins][NptBins],		param_lph_PX_sigmaerr[NrapBins][NptBins];
	  double param_ltp_PX_mean[NrapBins][NptBins],	param_ltp_PX_meanerr[NrapBins][NptBins],			param_ltp_PX_sigma[NrapBins][NptBins],		param_ltp_PX_sigma_high[NrapBins][NptBins],		param_ltp_PX_sigma_low[NrapBins][NptBins],		param_ltp_PX_sigmaerr[NrapBins][NptBins];
	  double param_lthstar_PX_mean[NrapBins][NptBins],	param_lthstar_PX_meanerr[NrapBins][NptBins],	param_lthstar_PX_sigma[NrapBins][NptBins],	param_lthstar_PX_sigma_high[NrapBins][NptBins],	param_lthstar_PX_sigma_low[NrapBins][NptBins],	param_lthstar_PX_sigmaerr[NrapBins][NptBins];
	  double param_lphstar_PX_mean[NrapBins][NptBins],	param_lphstar_PX_meanerr[NrapBins][NptBins],	param_lphstar_PX_sigma[NrapBins][NptBins],	param_lphstar_PX_sigma_high[NrapBins][NptBins],	param_lphstar_PX_sigma_low[NrapBins][NptBins],	param_lphstar_PX_sigmaerr[NrapBins][NptBins];
	  double param_ltilde_PX_mean[NrapBins][NptBins],	param_ltilde_PX_meanerr[NrapBins][NptBins],		param_ltilde_PX_sigma[NrapBins][NptBins],	param_ltilde_PX_sigma_high[NrapBins][NptBins],	param_ltilde_PX_sigma_low[NrapBins][NptBins],	param_ltilde_PX_sigmaerr[NrapBins][NptBins];
...
///////////////////// End of Extraction ////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  if(RealData) sprintf(filename,"%s/results_%s_rap%d_pT%d.root",dirstruct,TreeID,iRap,iPt);
...
*/
    ...
    //  cout << endl << endl;
      ...
      err_ltilde_PX=(err_low_ltilde_PX+err_high_ltilde_PX)/2.;
		...
		if(RealData){
		     char filename[200];
			  sprintf(filename,"%s/Figures/PosteriorDist_lth_CS_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{CS}_{#theta}", filename, h_lth_CS, lth_CS, err_low_lth_CS, err_high_lth_CS);
			  sprintf(filename,"%s/Figures/PosteriorDist_lph_CS_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{CS}_{#phi}", filename, h_lph_CS, lph_CS, err_low_lph_CS, err_high_lph_CS);
			  sprintf(filename,"%s/Figures/PosteriorDist_ltp_CS_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{CS}_{#theta#phi}", filename, h_ltp_CS, ltp_CS, err_low_ltp_CS, err_high_ltp_CS);
		      sprintf(filename,"%s/Figures/PosteriorDist_ltilde_CS_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#tilde{#lambda}^{CS}", filename, h_ltilde_CS, ltilde_CS, err_low_ltilde_CS, err_high_ltilde_CS);
			  sprintf(filename,"%s/Figures/PosteriorDist_lthstar_CS_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{*CS}_{#theta}", filename, h_lthstar_CS, lthstar_CS, err_low_lthstar_CS, err_high_lthstar_CS);
			  sprintf(filename,"%s/Figures/PosteriorDist_lphstar_CS_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{*CS}_{#phi}", filename, h_lphstar_CS, lphstar_CS, err_low_lphstar_CS, err_high_lphstar_CS);

			  sprintf(filename,"%s/Figures/PosteriorDist_lth_HX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{HX}_{#theta}", filename, h_lth_HX, lth_HX, err_low_lth_HX, err_high_lth_HX);
			  sprintf(filename,"%s/Figures/PosteriorDist_lph_HX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{HX}_{#phi}", filename, h_lph_HX, lph_HX, err_low_lph_HX, err_high_lph_HX);
			  sprintf(filename,"%s/Figures/PosteriorDist_ltp_HX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{HX}_{#theta#phi}", filename, h_ltp_HX, ltp_HX, err_low_ltp_HX, err_high_ltp_HX);
		      sprintf(filename,"%s/Figures/PosteriorDist_ltilde_HX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#tilde{#lambda}^{HX}", filename, h_ltilde_HX, ltilde_HX, err_low_ltilde_HX, err_high_ltilde_HX);
			  sprintf(filename,"%s/Figures/PosteriorDist_lthstar_HX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{*HX}_{#theta}", filename, h_lthstar_HX, lthstar_HX, err_low_lthstar_HX, err_high_lthstar_HX);
			  sprintf(filename,"%s/Figures/PosteriorDist_lphstar_HX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{*HX}_{#phi}", filename, h_lphstar_HX, lphstar_HX, err_low_lphstar_HX, err_high_lphstar_HX);

			  sprintf(filename,"%s/Figures/PosteriorDist_lth_PX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{PX}_{#theta}", filename, h_lth_PX, lth_PX, err_low_lth_PX, err_high_lth_PX);
			  sprintf(filename,"%s/Figures/PosteriorDist_lph_PX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{PX}_{#phi}", filename, h_lph_PX, lph_PX, err_low_lph_PX, err_high_lph_PX);
			  sprintf(filename,"%s/Figures/PosteriorDist_ltp_PX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{PX}_{#theta#phi}", filename, h_ltp_PX, ltp_PX, err_low_ltp_PX, err_high_ltp_PX);
		      sprintf(filename,"%s/Figures/PosteriorDist_ltilde_PX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#tilde{#lambda}^{PX}", filename, h_ltilde_PX, ltilde_PX, err_low_ltilde_PX, err_high_ltilde_PX);
			  sprintf(filename,"%s/Figures/PosteriorDist_lthstar_PX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{*PX}_{#theta}", filename, h_lthstar_PX, lthstar_PX, err_low_lthstar_PX, err_high_lthstar_PX);
			  sprintf(filename,"%s/Figures/PosteriorDist_lphstar_PX_%s_rap%d_pT%d.pdf",dirstruct,TreeID,iRap,iPt,MPValgo);
		      PlotPosterior(iPt, iRap, "#lambda^{*PX}_{#phi}", filename, h_lphstar_PX, lphstar_PX, err_low_lphstar_PX, err_high_lphstar_PX);
	  ...
	  if(nGen_[rap-1][pt-1]==0) {emptyBin[rap-1][pt-1]=true; cout<<"empty bin"<<endl;}
	    ...
	    param_lth_CS_mean[rap-1][pt-1]=param_lth_CS->GetMean(); param_lth_CS_meanerr[rap-1][pt-1]=param_lth_CS->GetMeanError();						 param_lth_CS_sigma[rap-1][pt-1]=err_param_lth_CS->GetMean(); 				param_lth_CS_sigma_high[rap-1][pt-1]=err_high_param_lth_CS->GetMean(); 				param_lth_CS_sigma_low[rap-1][pt-1]=err_low_param_lth_CS->GetMean(); 					param_lth_CS_sigmaerr[rap-1][pt-1]=err_param_lth_CS->GetMeanError(); 											if(RealData) {param_lth_CS_mean[rap-1][pt-1]=lth_CS; 			param_lth_CS_sigma[rap-1][pt-1]=err_lth_CS;        		param_lth_CS_sigma_high[rap-1][pt-1]=err_high_lth_CS;        		param_lth_CS_sigma_low[rap-1][pt-1]=err_low_lth_CS;        		}
	    param_lph_CS_mean[rap-1][pt-1]=param_lph_CS->GetMean(); param_lph_CS_meanerr[rap-1][pt-1]=param_lph_CS->GetMeanError();						 param_lph_CS_sigma[rap-1][pt-1]=err_param_lph_CS->GetMean(); 				param_lph_CS_sigma_high[rap-1][pt-1]=err_high_param_lph_CS->GetMean(); 				param_lph_CS_sigma_low[rap-1][pt-1]=err_low_param_lph_CS->GetMean(); 					param_lph_CS_sigmaerr[rap-1][pt-1]=err_param_lph_CS->GetMeanError(); 											if(RealData) {param_lph_CS_mean[rap-1][pt-1]=lph_CS; 			param_lph_CS_sigma[rap-1][pt-1]=err_lph_CS;        		param_lph_CS_sigma_high[rap-1][pt-1]=err_high_lph_CS;        		param_lph_CS_sigma_low[rap-1][pt-1]=err_low_lph_CS;        		}
	    param_ltp_CS_mean[rap-1][pt-1]=param_ltp_CS->GetMean(); param_ltp_CS_meanerr[rap-1][pt-1]=param_ltp_CS->GetMeanError();						 param_ltp_CS_sigma[rap-1][pt-1]=err_param_ltp_CS->GetMean(); 				param_ltp_CS_sigma_high[rap-1][pt-1]=err_high_param_ltp_CS->GetMean(); 				param_ltp_CS_sigma_low[rap-1][pt-1]=err_low_param_ltp_CS->GetMean(); 					param_ltp_CS_sigmaerr[rap-1][pt-1]=err_param_ltp_CS->GetMeanError(); 											if(RealData) {param_ltp_CS_mean[rap-1][pt-1]=ltp_CS; 			param_ltp_CS_sigma[rap-1][pt-1]=err_ltp_CS;        		param_ltp_CS_sigma_high[rap-1][pt-1]=err_high_ltp_CS;        		param_ltp_CS_sigma_low[rap-1][pt-1]=err_low_ltp_CS;        		}
	    param_ltilde_CS_mean[rap-1][pt-1]=param_ltilde_CS->GetMean(); param_ltilde_CS_meanerr[rap-1][pt-1]=param_ltilde_CS->GetMeanError();						 param_ltilde_CS_sigma[rap-1][pt-1]=err_param_ltilde_CS->GetMean(); 				param_ltilde_CS_sigma_high[rap-1][pt-1]=err_high_param_ltilde_CS->GetMean(); 				param_ltilde_CS_sigma_low[rap-1][pt-1]=err_low_param_ltilde_CS->GetMean(); 					param_ltilde_CS_sigmaerr[rap-1][pt-1]=err_param_ltilde_CS->GetMeanError(); 											if(RealData) {param_ltilde_CS_mean[rap-1][pt-1]=ltilde_CS; 			param_ltilde_CS_sigma[rap-1][pt-1]=err_ltilde_CS;        		param_ltilde_CS_sigma_high[rap-1][pt-1]=err_high_ltilde_CS;        		param_ltilde_CS_sigma_low[rap-1][pt-1]=err_low_ltilde_CS;        		}
	    param_lphstar_CS_mean[rap-1][pt-1]=param_lphstar_CS->GetMean(); param_lphstar_CS_meanerr[rap-1][pt-1]=param_lphstar_CS->GetMeanError();						 param_lphstar_CS_sigma[rap-1][pt-1]=err_param_lphstar_CS->GetMean(); 				param_lphstar_CS_sigma_high[rap-1][pt-1]=err_high_param_lphstar_CS->GetMean(); 				param_lphstar_CS_sigma_low[rap-1][pt-1]=err_low_param_lphstar_CS->GetMean(); 					param_lphstar_CS_sigmaerr[rap-1][pt-1]=err_param_lphstar_CS->GetMeanError(); 											if(RealData) {param_lphstar_CS_mean[rap-1][pt-1]=lphstar_CS; 			param_lphstar_CS_sigma[rap-1][pt-1]=err_lphstar_CS;        		param_lphstar_CS_sigma_high[rap-1][pt-1]=err_high_lphstar_CS;        		param_lphstar_CS_sigma_low[rap-1][pt-1]=err_low_lphstar_CS;        		}
	    param_lthstar_CS_mean[rap-1][pt-1]=param_lthstar_CS->GetMean(); param_lthstar_CS_meanerr[rap-1][pt-1]=param_lthstar_CS->GetMeanError();						 param_lthstar_CS_sigma[rap-1][pt-1]=err_param_lthstar_CS->GetMean(); 				param_lthstar_CS_sigma_high[rap-1][pt-1]=err_high_param_lthstar_CS->GetMean(); 				param_lthstar_CS_sigma_low[rap-1][pt-1]=err_low_param_lthstar_CS->GetMean(); 					param_lthstar_CS_sigmaerr[rap-1][pt-1]=err_param_lthstar_CS->GetMeanError(); 											if(RealData) {param_lthstar_CS_mean[rap-1][pt-1]=lthstar_CS; 			param_lthstar_CS_sigma[rap-1][pt-1]=err_lthstar_CS;        		param_lthstar_CS_sigma_high[rap-1][pt-1]=err_high_lthstar_CS;        		param_lthstar_CS_sigma_low[rap-1][pt-1]=err_low_lthstar_CS;        		}

	    param_lth_HX_mean[rap-1][pt-1]=param_lth_HX->GetMean(); param_lth_HX_meanerr[rap-1][pt-1]=param_lth_HX->GetMeanError();						 param_lth_HX_sigma[rap-1][pt-1]=err_param_lth_HX->GetMean(); 				param_lth_HX_sigma_high[rap-1][pt-1]=err_high_param_lth_HX->GetMean(); 				param_lth_HX_sigma_low[rap-1][pt-1]=err_low_param_lth_HX->GetMean(); 					param_lth_HX_sigmaerr[rap-1][pt-1]=err_param_lth_HX->GetMeanError(); 											if(RealData) {param_lth_HX_mean[rap-1][pt-1]=lth_HX; 			param_lth_HX_sigma[rap-1][pt-1]=err_lth_HX;        		param_lth_HX_sigma_high[rap-1][pt-1]=err_high_lth_HX;        		param_lth_HX_sigma_low[rap-1][pt-1]=err_low_lth_HX;        		}
	    param_lph_HX_mean[rap-1][pt-1]=param_lph_HX->GetMean(); param_lph_HX_meanerr[rap-1][pt-1]=param_lph_HX->GetMeanError();						 param_lph_HX_sigma[rap-1][pt-1]=err_param_lph_HX->GetMean(); 				param_lph_HX_sigma_high[rap-1][pt-1]=err_high_param_lph_HX->GetMean(); 				param_lph_HX_sigma_low[rap-1][pt-1]=err_low_param_lph_HX->GetMean(); 					param_lph_HX_sigmaerr[rap-1][pt-1]=err_param_lph_HX->GetMeanError(); 											if(RealData) {param_lph_HX_mean[rap-1][pt-1]=lph_HX; 			param_lph_HX_sigma[rap-1][pt-1]=err_lph_HX;        		param_lph_HX_sigma_high[rap-1][pt-1]=err_high_lph_HX;        		param_lph_HX_sigma_low[rap-1][pt-1]=err_low_lph_HX;        		}
	    param_ltp_HX_mean[rap-1][pt-1]=param_ltp_HX->GetMean(); param_ltp_HX_meanerr[rap-1][pt-1]=param_ltp_HX->GetMeanError();						 param_ltp_HX_sigma[rap-1][pt-1]=err_param_ltp_HX->GetMean(); 				param_ltp_HX_sigma_high[rap-1][pt-1]=err_high_param_ltp_HX->GetMean(); 				param_ltp_HX_sigma_low[rap-1][pt-1]=err_low_param_ltp_HX->GetMean(); 					param_ltp_HX_sigmaerr[rap-1][pt-1]=err_param_ltp_HX->GetMeanError(); 											if(RealData) {param_ltp_HX_mean[rap-1][pt-1]=ltp_HX; 			param_ltp_HX_sigma[rap-1][pt-1]=err_ltp_HX;        		param_ltp_HX_sigma_high[rap-1][pt-1]=err_high_ltp_HX;        		param_ltp_HX_sigma_low[rap-1][pt-1]=err_low_ltp_HX;        		}
	    param_ltilde_HX_mean[rap-1][pt-1]=param_ltilde_HX->GetMean(); param_ltilde_HX_meanerr[rap-1][pt-1]=param_ltilde_HX->GetMeanError();						 param_ltilde_HX_sigma[rap-1][pt-1]=err_param_ltilde_HX->GetMean(); 				param_ltilde_HX_sigma_high[rap-1][pt-1]=err_high_param_ltilde_HX->GetMean(); 				param_ltilde_HX_sigma_low[rap-1][pt-1]=err_low_param_ltilde_HX->GetMean(); 					param_ltilde_HX_sigmaerr[rap-1][pt-1]=err_param_ltilde_HX->GetMeanError(); 											if(RealData) {param_ltilde_HX_mean[rap-1][pt-1]=ltilde_HX; 			param_ltilde_HX_sigma[rap-1][pt-1]=err_ltilde_HX;        		param_ltilde_HX_sigma_high[rap-1][pt-1]=err_high_ltilde_HX;        		param_ltilde_HX_sigma_low[rap-1][pt-1]=err_low_ltilde_HX;        		}
	    param_lphstar_HX_mean[rap-1][pt-1]=param_lphstar_HX->GetMean(); param_lphstar_HX_meanerr[rap-1][pt-1]=param_lphstar_HX->GetMeanError();						 param_lphstar_HX_sigma[rap-1][pt-1]=err_param_lphstar_HX->GetMean(); 				param_lphstar_HX_sigma_high[rap-1][pt-1]=err_high_param_lphstar_HX->GetMean(); 				param_lphstar_HX_sigma_low[rap-1][pt-1]=err_low_param_lphstar_HX->GetMean(); 					param_lphstar_HX_sigmaerr[rap-1][pt-1]=err_param_lphstar_HX->GetMeanError(); 											if(RealData) {param_lphstar_HX_mean[rap-1][pt-1]=lphstar_HX; 			param_lphstar_HX_sigma[rap-1][pt-1]=err_lphstar_HX;        		param_lphstar_HX_sigma_high[rap-1][pt-1]=err_high_lphstar_HX;        		param_lphstar_HX_sigma_low[rap-1][pt-1]=err_low_lphstar_HX;        		}
	    param_lthstar_HX_mean[rap-1][pt-1]=param_lthstar_HX->GetMean(); param_lthstar_HX_meanerr[rap-1][pt-1]=param_lthstar_HX->GetMeanError();						 param_lthstar_HX_sigma[rap-1][pt-1]=err_param_lthstar_HX->GetMean(); 				param_lthstar_HX_sigma_high[rap-1][pt-1]=err_high_param_lthstar_HX->GetMean(); 				param_lthstar_HX_sigma_low[rap-1][pt-1]=err_low_param_lthstar_HX->GetMean(); 					param_lthstar_HX_sigmaerr[rap-1][pt-1]=err_param_lthstar_HX->GetMeanError(); 											if(RealData) {param_lthstar_HX_mean[rap-1][pt-1]=lthstar_HX; 			param_lthstar_HX_sigma[rap-1][pt-1]=err_lthstar_HX;        		param_lthstar_HX_sigma_high[rap-1][pt-1]=err_high_lthstar_HX;        		param_lthstar_HX_sigma_low[rap-1][pt-1]=err_low_lthstar_HX;        		}

	    param_lth_PX_mean[rap-1][pt-1]=param_lth_PX->GetMean(); param_lth_PX_meanerr[rap-1][pt-1]=param_lth_PX->GetMeanError();						 param_lth_PX_sigma[rap-1][pt-1]=err_param_lth_PX->GetMean(); 				param_lth_PX_sigma_high[rap-1][pt-1]=err_high_param_lth_PX->GetMean(); 				param_lth_PX_sigma_low[rap-1][pt-1]=err_low_param_lth_PX->GetMean(); 					param_lth_PX_sigmaerr[rap-1][pt-1]=err_param_lth_PX->GetMeanError(); 											if(RealData) {param_lth_PX_mean[rap-1][pt-1]=lth_PX; 			param_lth_PX_sigma[rap-1][pt-1]=err_lth_PX;        		param_lth_PX_sigma_high[rap-1][pt-1]=err_high_lth_PX;        		param_lth_PX_sigma_low[rap-1][pt-1]=err_low_lth_PX;        		}
	    param_lph_PX_mean[rap-1][pt-1]=param_lph_PX->GetMean(); param_lph_PX_meanerr[rap-1][pt-1]=param_lph_PX->GetMeanError();						 param_lph_PX_sigma[rap-1][pt-1]=err_param_lph_PX->GetMean(); 				param_lph_PX_sigma_high[rap-1][pt-1]=err_high_param_lph_PX->GetMean(); 				param_lph_PX_sigma_low[rap-1][pt-1]=err_low_param_lph_PX->GetMean(); 					param_lph_PX_sigmaerr[rap-1][pt-1]=err_param_lph_PX->GetMeanError(); 											if(RealData) {param_lph_PX_mean[rap-1][pt-1]=lph_PX; 			param_lph_PX_sigma[rap-1][pt-1]=err_lph_PX;        		param_lph_PX_sigma_high[rap-1][pt-1]=err_high_lph_PX;        		param_lph_PX_sigma_low[rap-1][pt-1]=err_low_lph_PX;        		}
	    param_ltp_PX_mean[rap-1][pt-1]=param_ltp_PX->GetMean(); param_ltp_PX_meanerr[rap-1][pt-1]=param_ltp_PX->GetMeanError();						 param_ltp_PX_sigma[rap-1][pt-1]=err_param_ltp_PX->GetMean(); 				param_ltp_PX_sigma_high[rap-1][pt-1]=err_high_param_ltp_PX->GetMean(); 				param_ltp_PX_sigma_low[rap-1][pt-1]=err_low_param_ltp_PX->GetMean(); 					param_ltp_PX_sigmaerr[rap-1][pt-1]=err_param_ltp_PX->GetMeanError(); 											if(RealData) {param_ltp_PX_mean[rap-1][pt-1]=ltp_PX; 			param_ltp_PX_sigma[rap-1][pt-1]=err_ltp_PX;        		param_ltp_PX_sigma_high[rap-1][pt-1]=err_high_ltp_PX;        		param_ltp_PX_sigma_low[rap-1][pt-1]=err_low_ltp_PX;        		}
	    param_ltilde_PX_mean[rap-1][pt-1]=param_ltilde_PX->GetMean(); param_ltilde_PX_meanerr[rap-1][pt-1]=param_ltilde_PX->GetMeanError();						 param_ltilde_PX_sigma[rap-1][pt-1]=err_param_ltilde_PX->GetMean(); 				param_ltilde_PX_sigma_high[rap-1][pt-1]=err_high_param_ltilde_PX->GetMean(); 				param_ltilde_PX_sigma_low[rap-1][pt-1]=err_low_param_ltilde_PX->GetMean(); 					param_ltilde_PX_sigmaerr[rap-1][pt-1]=err_param_ltilde_PX->GetMeanError(); 											if(RealData) {param_ltilde_PX_mean[rap-1][pt-1]=ltilde_PX; 			param_ltilde_PX_sigma[rap-1][pt-1]=err_ltilde_PX;        		param_ltilde_PX_sigma_high[rap-1][pt-1]=err_high_ltilde_PX;        		param_ltilde_PX_sigma_low[rap-1][pt-1]=err_low_ltilde_PX;        		}
	    param_lphstar_PX_mean[rap-1][pt-1]=param_lphstar_PX->GetMean(); param_lphstar_PX_meanerr[rap-1][pt-1]=param_lphstar_PX->GetMeanError();						 param_lphstar_PX_sigma[rap-1][pt-1]=err_param_lphstar_PX->GetMean(); 				param_lphstar_PX_sigma_high[rap-1][pt-1]=err_high_param_lphstar_PX->GetMean(); 				param_lphstar_PX_sigma_low[rap-1][pt-1]=err_low_param_lphstar_PX->GetMean(); 					param_lphstar_PX_sigmaerr[rap-1][pt-1]=err_param_lphstar_PX->GetMeanError(); 											if(RealData) {param_lphstar_PX_mean[rap-1][pt-1]=lphstar_PX; 			param_lphstar_PX_sigma[rap-1][pt-1]=err_lphstar_PX;        		param_lphstar_PX_sigma_high[rap-1][pt-1]=err_high_lphstar_PX;        		param_lphstar_PX_sigma_low[rap-1][pt-1]=err_low_lphstar_PX;        		}
	    param_lthstar_PX_mean[rap-1][pt-1]=param_lthstar_PX->GetMean(); param_lthstar_PX_meanerr[rap-1][pt-1]=param_lthstar_PX->GetMeanError();						 param_lthstar_PX_sigma[rap-1][pt-1]=err_param_lthstar_PX->GetMean(); 				param_lthstar_PX_sigma_high[rap-1][pt-1]=err_high_param_lthstar_PX->GetMean(); 				param_lthstar_PX_sigma_low[rap-1][pt-1]=err_low_param_lthstar_PX->GetMean(); 					param_lthstar_PX_sigmaerr[rap-1][pt-1]=err_param_lthstar_PX->GetMeanError(); 											if(RealData) {param_lthstar_PX_mean[rap-1][pt-1]=lthstar_PX; 			param_lthstar_PX_sigma[rap-1][pt-1]=err_lthstar_PX;        		param_lthstar_PX_sigma_high[rap-1][pt-1]=err_high_lthstar_PX;        		param_lthstar_PX_sigma_low[rap-1][pt-1]=err_low_lthstar_PX;        		}
	    ...
	    */
	    ...
	    if(!RealData&&UseMedianForToys){
		    ...
		    param_lth_CS->GetQuantiles(1,&quantile,&prob); param_lth_CS_mean[rap-1][pt-1]=quantile;
		    param_lph_CS->GetQuantiles(1,&quantile,&prob); param_lph_CS_mean[rap-1][pt-1]=quantile;
		    param_ltp_CS->GetQuantiles(1,&quantile,&prob); param_ltp_CS_mean[rap-1][pt-1]=quantile;
		    param_ltilde_CS->GetQuantiles(1,&quantile,&prob); param_ltilde_CS_mean[rap-1][pt-1]=quantile;

		    param_lth_HX->GetQuantiles(1,&quantile,&prob); param_lth_HX_mean[rap-1][pt-1]=quantile;
		    param_lph_HX->GetQuantiles(1,&quantile,&prob); param_lph_HX_mean[rap-1][pt-1]=quantile;
		    param_ltp_HX->GetQuantiles(1,&quantile,&prob); param_ltp_HX_mean[rap-1][pt-1]=quantile;
		    param_ltilde_HX->GetQuantiles(1,&quantile,&prob); param_ltilde_HX_mean[rap-1][pt-1]=quantile;

		    param_lth_PX->GetQuantiles(1,&quantile,&prob); param_lth_PX_mean[rap-1][pt-1]=quantile;
		    param_lph_PX->GetQuantiles(1,&quantile,&prob); param_lph_PX_mean[rap-1][pt-1]=quantile;
		    param_ltp_PX->GetQuantiles(1,&quantile,&prob); param_ltp_PX_mean[rap-1][pt-1]=quantile;
		    param_ltilde_PX->GetQuantiles(1,&quantile,&prob); param_ltilde_PX_mean[rap-1][pt-1]=quantile;
...
//////////////////////////////////////////////////
	    ...
	    double KinDepRaperr_low[NptBins];
	    double KinDepRaperr_high[NptBins];
	    double KinDepRap2[NptBins];
	    double KinDepRap2err_low[NptBins];
	    double KinDepRap2err_high[NptBins];
	    double KinDepRap3[NptBins];
	    double KinDepRap3err_low[NptBins];
	    double KinDepRap3err_high[NptBins];
	    ...
	    for(int rapBin = rapBinMin; rapBin < rapBinMax+1; rapBin++) {
	    	...
	    	if(iLam==13){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_CS_mean_lth_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{CS}_{#theta}");																sprintf(GraphName,"lth_CS_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==15){lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_CS_mean_lph_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{CS}_{#phi}");																sprintf(GraphName,"lph_CS_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==17){lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_CS_mean_ltp_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{CS}_{#theta#phi}");															sprintf(GraphName,"ltp_CS_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==19){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_CS_mean_lthstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{*CS}_{#theta}");														sprintf(GraphName,"lthstar_CS_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==21){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_CS_mean_lphstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{*CS}_{#phi}");															sprintf(GraphName,"lphstar_CS_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==23){lammin=dmumin;		lammax=dmumax;	if(polScen==6){lammin=lammin+2;lammax=lammax+2;}sprintf(filename,"%s/Figures/KinDep_param_CS_mean_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#tilde{#lambda}^{CS}");				sprintf(GraphName,"ltilde_CS_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	if(iLam==24){lammin=dsigmin;	lammax=dsigmax;	if(polScen==6){lammin=lammin+2;lammax=lammax+2;}sprintf(filename,"%s/Figures/KinDep_param_CS_sigma_ltilde_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#sigma_{#tilde{#lambda}^{CS}}");				}

	    	if(iLam==25){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_HX_mean_lth_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{HX}_{#theta}");                                                            sprintf(GraphName,"lth_HX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==27){lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_HX_mean_lph_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{HX}_{#phi}");                                                              sprintf(GraphName,"lph_HX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==29){lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_HX_mean_ltp_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{HX}_{#theta#phi}");                                                        sprintf(GraphName,"ltp_HX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==31){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_HX_mean_lthstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{*HX}_{#theta}");                                                       sprintf(GraphName,"lthstar_HX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==33){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_HX_mean_lphstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{*HX}_{#phi}");                                                         sprintf(GraphName,"lphstar_HX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==35){lammin=dmumin;		lammax=dmumax;	if(polScen==6){lammin=lammin+2;lammax=lammax+2;}sprintf(filename,"%s/Figures/KinDep_param_HX_mean_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#tilde{#lambda}^{HX}");             sprintf(GraphName,"ltilde_HX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	if(iLam==36){lammin=dsigmin;	lammax=dsigmax;	if(polScen==6){lammin=lammin+2;lammax=lammax+2;}sprintf(filename,"%s/Figures/KinDep_param_HX_sigma_ltilde_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#sigma_{#tilde{#lambda}^{HX}}");    		}

	    	if(iLam==37){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_PX_mean_lth_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{PX}_{#theta}");                                                            sprintf(GraphName,"lth_PX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==39){lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_PX_mean_lph_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{PX}_{#phi}");                                                              sprintf(GraphName,"lph_PX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==41){lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_PX_mean_ltp_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{PX}_{#theta#phi}");                                                        sprintf(GraphName,"ltp_PX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==43){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_PX_mean_lthstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{*PX}_{#theta}");                                                       sprintf(GraphName,"lthstar_PX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==45){lammin=dmumin;		lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_PX_mean_lphstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#lambda^{*PX}_{#phi}");                                                         sprintf(GraphName,"lphstar_PX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	...
	    	if(iLam==47){lammin=dmumin;		lammax=dmumax;	if(polScen==6){lammin=lammin+2;lammax=lammax+2;}sprintf(filename,"%s/Figures/KinDep_param_PX_mean_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#tilde{#lambda}^{PX}");             sprintf(GraphName,"ltilde_PX_rap%d",rapBin); if(RealData) SaveGraph=true;		}
	    	if(iLam==48){lammin=dsigmin;	lammax=dsigmax;	if(polScen==6){lammin=lammin+2;lammax=lammax+2;}sprintf(filename,"%s/Figures/KinDep_param_PX_sigma_ltilde_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#sigma_{#tilde{#lambda}^{PX}}");    		}
	    	...
	    	if(iLam==85) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_CS_deltamean_lth_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{CS}_{#theta}}");		DrawLine=true;lamline=0;	sprintf(GraphName,"lth_CS_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==86) {lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_CS_deltamean_lph_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{CS}_{#phi}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"lph_CS_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==87) {lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_CS_deltamean_ltp_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{CS}_{#theta#phi}}");	DrawLine=true;lamline=0;	sprintf(GraphName,"ltp_CS_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==88) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_CS_deltamean_lthstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#lambda^{*CS}_{#theta}}");		DrawLine=true;lamline=0;	sprintf(GraphName,"lthstar_CS_rap%d",rapBin); if(!RealData) SaveGraph=true;	}
	    	if(iLam==89) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_CS_deltamean_lphstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#lambda^{*CS}_{#phi}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"lphstar_CS_rap%d",rapBin); if(!RealData) SaveGraph=true;	}
	    	if(iLam==90) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_CS_deltamean_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#tilde{#lambda}^{CS}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"ltilde_CS_rap%d",rapBin); if(!RealData) SaveGraph=true;	}

	    	if(iLam==91) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_HX_deltamean_lth_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{HX}_{#theta}}");		DrawLine=true;lamline=0;	sprintf(GraphName,"lth_HX_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==92) {lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_HX_deltamean_lph_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{HX}_{#phi}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"lph_HX_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==93) {lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_HX_deltamean_ltp_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{HX}_{#theta#phi}}");	DrawLine=true;lamline=0;	sprintf(GraphName,"ltp_HX_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==94) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_HX_deltamean_lthstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#lambda^{*HX}_{#theta}}");		DrawLine=true;lamline=0;	sprintf(GraphName,"lthstar_HX_rap%d",rapBin); if(!RealData) SaveGraph=true;	}
	    	if(iLam==95) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_HX_deltamean_lphstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#lambda^{*HX}_{#phi}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"lphstar_HX_rap%d",rapBin); if(!RealData) SaveGraph=true;	}
	    	if(iLam==96) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_HX_deltamean_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#tilde{#lambda}^{HX}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"ltilde_HX_rap%d",rapBin); if(!RealData) SaveGraph=true;	}

	    	if(iLam==97) {lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_PX_deltamean_lth_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{PX}_{#theta}}");		DrawLine=true;lamline=0;	sprintf(GraphName,"lth_PX_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==98) {lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_PX_deltamean_lph_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{PX}_{#phi}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"lph_PX_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==99) {lammin=thetaphimin;		lammax=thetaphimax;	sprintf(filename,"%s/Figures/KinDep_param_PX_deltamean_ltp_rap%d.pdf",dirstruct,rapBin); 			sprintf(axislabel,"#delta_{#lambda^{PX}_{#theta#phi}}");	DrawLine=true;lamline=0;	sprintf(GraphName,"ltp_PX_rap%d",rapBin); if(!RealData) SaveGraph=true;		}
	    	if(iLam==100){lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_PX_deltamean_lthstar_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#delta_{#lambda^{*PX}_{#theta}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"lthstar_PX_rap%d",rapBin); if(!RealData) SaveGraph=true;	}
	    	if(iLam==101){lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_PX_deltamean_lphstar_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#lambda^{*PX}_{#phi}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"lphstar_PX_rap%d",rapBin); if(!RealData) SaveGraph=true;	}
	    	if(iLam==102){lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_PX_deltamean_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#delta_{#tilde{#lambda}^{PX}}");			DrawLine=true;lamline=0;	sprintf(GraphName,"ltilde_PX_rap%d",rapBin); if(!RealData) SaveGraph=true;	}

	    	if(iLam==103){lammin=tildeCompmin;	lammax=tildeCompmax;	if(polScen==6){lammin=lammin+2;lammax=lammax+2;}sprintf(filename,"%s/Figures/KinDep_param_mean_Comparison_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#tilde{#lambda}"); lamline=lambda_tilde_injected_CS[rapBin-1][0];}
	    	if(iLam==104){lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_mean_Comparison_lthstar_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#lambda*_{#theta}"); lamline=lambda_thetastar_injected_CS[rapBin-1][0];}
	    	if(iLam==105){lammin=dmumin;	lammax=dmumax;	sprintf(filename,"%s/Figures/KinDep_param_mean_Comparison_lphstar_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#lambda*_{#phi}"); lamline=lambda_phistar_injected_CS[rapBin-1][0];}
					...
					for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
						...
						if(iLam==13){ KinDepRap[pt]= param_lth_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lth_CS_meanerr[rap][pt];		 	if(RealData) {KinDepRaperr_low[pt]= param_lth_CS_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_lth_CS_sigma_high[rap][pt];}    }
						if(iLam==14){ KinDepRap[pt]= param_lth_CS_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_lth_CS_sigmaerr[rap][pt];		}
						if(iLam==15){ KinDepRap[pt]= param_lph_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lph_CS_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_lph_CS_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_lph_CS_sigma_high[rap][pt];}    }
						if(iLam==16){ KinDepRap[pt]= param_lph_CS_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_lph_CS_sigmaerr[rap][pt];		}
						if(iLam==17){ KinDepRap[pt]= param_ltp_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_CS_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_ltp_CS_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_ltp_CS_sigma_high[rap][pt];}    }
						if(iLam==18){ KinDepRap[pt]= param_ltp_CS_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_CS_sigmaerr[rap][pt];		}
						if(iLam==19){ KinDepRap[pt]= param_lthstar_CS_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_CS_meanerr[rap][pt]; 	if(RealData) {KinDepRaperr_low[pt]= param_lthstar_CS_sigma_low[rap][pt];	KinDepRaperr_high[pt]= param_lthstar_CS_sigma_high[rap][pt];}}
						if(iLam==20){ KinDepRap[pt]= param_lthstar_CS_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_CS_sigmaerr[rap][pt];	}
						if(iLam==21){ KinDepRap[pt]= param_lphstar_CS_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_CS_meanerr[rap][pt]; 	if(RealData) {KinDepRaperr_low[pt]= param_lphstar_CS_sigma_low[rap][pt];	KinDepRaperr_high[pt]= param_lphstar_CS_sigma_high[rap][pt];}}
						if(iLam==22){ KinDepRap[pt]= param_lphstar_CS_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_CS_sigmaerr[rap][pt];	}
						if(iLam==23){ KinDepRap[pt]= param_ltilde_CS_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_ltilde_CS_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_ltilde_CS_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_ltilde_CS_sigma_high[rap][pt];} }
						if(iLam==24){ KinDepRap[pt]= param_ltilde_CS_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_ltilde_CS_sigmaerr[rap][pt];		}

						if(iLam==25){ KinDepRap[pt]= param_lth_HX_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lth_HX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_lth_HX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_lth_HX_sigma_high[rap][pt];}    }
						if(iLam==26){ KinDepRap[pt]= param_lth_HX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_lth_HX_sigmaerr[rap][pt];		}
						if(iLam==27){ KinDepRap[pt]= param_lph_HX_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lph_HX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_lph_HX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_lph_HX_sigma_high[rap][pt];}    }
						if(iLam==28){ KinDepRap[pt]= param_lph_HX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_lph_HX_sigmaerr[rap][pt];		}
						if(iLam==29){ KinDepRap[pt]= param_ltp_HX_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_HX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_ltp_HX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_ltp_HX_sigma_high[rap][pt];}    }
						if(iLam==30){ KinDepRap[pt]= param_ltp_HX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_HX_sigmaerr[rap][pt];		}
						if(iLam==31){ KinDepRap[pt]= param_lthstar_HX_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_HX_meanerr[rap][pt]; 	if(RealData) {KinDepRaperr_low[pt]= param_lthstar_HX_sigma_low[rap][pt];	KinDepRaperr_high[pt]= param_lthstar_HX_sigma_high[rap][pt];}}
						if(iLam==32){ KinDepRap[pt]= param_lthstar_HX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_HX_sigmaerr[rap][pt];	}
						if(iLam==33){ KinDepRap[pt]= param_lphstar_HX_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_HX_meanerr[rap][pt]; 	if(RealData) {KinDepRaperr_low[pt]= param_lphstar_HX_sigma_low[rap][pt];	KinDepRaperr_high[pt]= param_lphstar_HX_sigma_high[rap][pt];}}
						if(iLam==34){ KinDepRap[pt]= param_lphstar_HX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_HX_sigmaerr[rap][pt];	}
						if(iLam==35){ KinDepRap[pt]= param_ltilde_HX_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_ltilde_HX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_ltilde_HX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_ltilde_HX_sigma_high[rap][pt];} }
						if(iLam==36){ KinDepRap[pt]= param_ltilde_HX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_ltilde_HX_sigmaerr[rap][pt];		}

						if(iLam==37){ KinDepRap[pt]= param_lth_PX_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lth_PX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_lth_PX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_lth_PX_sigma_high[rap][pt];}    }
						if(iLam==38){ KinDepRap[pt]= param_lth_PX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_lth_PX_sigmaerr[rap][pt];		}
						if(iLam==39){ KinDepRap[pt]= param_lph_PX_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lph_PX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_lph_PX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_lph_PX_sigma_high[rap][pt];}    }
						if(iLam==40){ KinDepRap[pt]= param_lph_PX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_lph_PX_sigmaerr[rap][pt];		}
						if(iLam==41){ KinDepRap[pt]= param_ltp_PX_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_PX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_ltp_PX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_ltp_PX_sigma_high[rap][pt];}    }
						if(iLam==42){ KinDepRap[pt]= param_ltp_PX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_PX_sigmaerr[rap][pt];		}
						if(iLam==43){ KinDepRap[pt]= param_lthstar_PX_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_PX_meanerr[rap][pt];	 	if(RealData) {KinDepRaperr_low[pt]= param_lthstar_PX_sigma_low[rap][pt];	KinDepRaperr_high[pt]= param_lthstar_PX_sigma_high[rap][pt];}}
						if(iLam==44){ KinDepRap[pt]= param_lthstar_PX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_PX_sigmaerr[rap][pt];	}
						if(iLam==45){ KinDepRap[pt]= param_lphstar_PX_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_PX_meanerr[rap][pt]; 	if(RealData) {KinDepRaperr_low[pt]= param_lphstar_PX_sigma_low[rap][pt];	KinDepRaperr_high[pt]= param_lphstar_PX_sigma_high[rap][pt];}}
						if(iLam==46){ KinDepRap[pt]= param_lphstar_PX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_PX_sigmaerr[rap][pt];	}
						if(iLam==47){ KinDepRap[pt]= param_ltilde_PX_mean[rap][pt]; 	KinDepRaperr_low[pt]=param_ltilde_PX_meanerr[rap][pt]; 		if(RealData) {KinDepRaperr_low[pt]= param_ltilde_PX_sigma_low[rap][pt];		KinDepRaperr_high[pt]= param_ltilde_PX_sigma_high[rap][pt];} }
						if(iLam==48){ KinDepRap[pt]= param_ltilde_PX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=param_ltilde_PX_sigmaerr[rap][pt];		}

						if(iLam==49){ KinDepRap[pt]= pull_lth_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_lth_CS_meanerr[rap][pt];			}
						if(iLam==50){ KinDepRap[pt]= pull_lth_CS_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_lth_CS_sigmaerr[rap][pt];			}
						if(iLam==51){ KinDepRap[pt]= pull_lph_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_lph_CS_meanerr[rap][pt];			}
						if(iLam==52){ KinDepRap[pt]= pull_lph_CS_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_lph_CS_sigmaerr[rap][pt];			}
						if(iLam==53){ KinDepRap[pt]= pull_ltp_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltp_CS_meanerr[rap][pt];			}
						if(iLam==54){ KinDepRap[pt]= pull_ltp_CS_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltp_CS_sigmaerr[rap][pt];			}
						if(iLam==55){ KinDepRap[pt]= pull_lthstar_CS_mean[rap][pt]; 	KinDepRaperr_low[pt]=pull_lthstar_CS_meanerr[rap][pt];		}
						if(iLam==56){ KinDepRap[pt]= pull_lthstar_CS_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_lthstar_CS_sigmaerr[rap][pt];		}
						if(iLam==57){ KinDepRap[pt]= pull_lphstar_CS_mean[rap][pt]; 	KinDepRaperr_low[pt]=pull_lphstar_CS_meanerr[rap][pt];		}
						if(iLam==58){ KinDepRap[pt]= pull_lphstar_CS_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_lphstar_CS_sigmaerr[rap][pt];		}
						if(iLam==59){ KinDepRap[pt]= pull_ltilde_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltilde_CS_meanerr[rap][pt];		}
						if(iLam==60){ KinDepRap[pt]= pull_ltilde_CS_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_ltilde_CS_sigmaerr[rap][pt];		}

						if(iLam==61){ KinDepRap[pt]= pull_lth_HX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_lth_HX_meanerr[rap][pt];			}
						if(iLam==62){ KinDepRap[pt]= pull_lth_HX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_lth_HX_sigmaerr[rap][pt];			}
						if(iLam==63){ KinDepRap[pt]= pull_lph_HX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_lph_HX_meanerr[rap][pt];			}
						if(iLam==64){ KinDepRap[pt]= pull_lph_HX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_lph_HX_sigmaerr[rap][pt];			}
						if(iLam==65){ KinDepRap[pt]= pull_ltp_HX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltp_HX_meanerr[rap][pt];			}
						if(iLam==66){ KinDepRap[pt]= pull_ltp_HX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltp_HX_sigmaerr[rap][pt];			}
						if(iLam==67){ KinDepRap[pt]= pull_lthstar_HX_mean[rap][pt]; 	KinDepRaperr_low[pt]=pull_lthstar_HX_meanerr[rap][pt];		}
						if(iLam==68){ KinDepRap[pt]= pull_lthstar_HX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_lthstar_HX_sigmaerr[rap][pt];		}
						if(iLam==69){ KinDepRap[pt]= pull_lphstar_HX_mean[rap][pt]; 	KinDepRaperr_low[pt]=pull_lphstar_HX_meanerr[rap][pt];		}
						if(iLam==70){ KinDepRap[pt]= pull_lphstar_HX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_lphstar_HX_sigmaerr[rap][pt];		}
						if(iLam==71){ KinDepRap[pt]= pull_ltilde_HX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltilde_HX_meanerr[rap][pt];		}
						if(iLam==72){ KinDepRap[pt]= pull_ltilde_HX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_ltilde_HX_sigmaerr[rap][pt];		}

						if(iLam==73){ KinDepRap[pt]= pull_lth_PX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_lth_PX_meanerr[rap][pt];			}
						if(iLam==74){ KinDepRap[pt]= pull_lth_PX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_lth_PX_sigmaerr[rap][pt];			}
						if(iLam==75){ KinDepRap[pt]= pull_lph_PX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_lph_PX_meanerr[rap][pt];			}
						if(iLam==76){ KinDepRap[pt]= pull_lph_PX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_lph_PX_sigmaerr[rap][pt];			}
						if(iLam==77){ KinDepRap[pt]= pull_ltp_PX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltp_PX_meanerr[rap][pt];			}
						if(iLam==78){ KinDepRap[pt]= pull_ltp_PX_sigma[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltp_PX_sigmaerr[rap][pt];			}
						if(iLam==79){ KinDepRap[pt]= pull_lthstar_PX_mean[rap][pt]; 	KinDepRaperr_low[pt]=pull_lthstar_PX_meanerr[rap][pt];		}
						if(iLam==80){ KinDepRap[pt]= pull_lthstar_PX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_lthstar_PX_sigmaerr[rap][pt];		}
						if(iLam==81){ KinDepRap[pt]= pull_lphstar_PX_mean[rap][pt]; 	KinDepRaperr_low[pt]=pull_lphstar_PX_meanerr[rap][pt];		}
						if(iLam==82){ KinDepRap[pt]= pull_lphstar_PX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_lphstar_PX_sigmaerr[rap][pt];		}
						if(iLam==83){ KinDepRap[pt]= pull_ltilde_PX_mean[rap][pt]; 		KinDepRaperr_low[pt]=pull_ltilde_PX_meanerr[rap][pt];		}
						if(iLam==84){ KinDepRap[pt]= pull_ltilde_PX_sigma[rap][pt]; 	KinDepRaperr_low[pt]=pull_ltilde_PX_sigmaerr[rap][pt];		}


						if(iLam==85){ KinDepRap[pt]= param_lth_CS_mean[rap][pt]-lambda_theta_injected_CS[rap][pt]; 			KinDepRaperr_low[pt]=param_lth_CS_sigma[rap][pt];}
						if(iLam==86){ KinDepRap[pt]= param_lph_CS_mean[rap][pt]-lambda_phi_injected_CS[rap][pt]; 			KinDepRaperr_low[pt]=param_lph_CS_sigma[rap][pt];}
						if(iLam==87){ KinDepRap[pt]= param_ltp_CS_mean[rap][pt]-lambda_thetaphi_injected_CS[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_CS_sigma[rap][pt];}
						if(iLam==88){ KinDepRap[pt]= param_lthstar_CS_mean[rap][pt]-lambda_thetastar_injected_CS[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_CS_sigma[rap][pt];}
						if(iLam==89){ KinDepRap[pt]= param_lphstar_CS_mean[rap][pt]-lambda_phistar_injected_CS[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_CS_sigma[rap][pt];}
						if(iLam==90){ KinDepRap[pt]= param_ltilde_CS_mean[rap][pt]-lambda_tilde_injected_CS[rap][pt]; 		KinDepRaperr_low[pt]=param_ltilde_CS_sigma[rap][pt];}

						if(iLam==91){ KinDepRap[pt]= param_lth_HX_mean[rap][pt]-lambda_theta_injected_HX[rap][pt]; 			KinDepRaperr_low[pt]=param_lth_HX_sigma[rap][pt];}
						if(iLam==92){ KinDepRap[pt]= param_lph_HX_mean[rap][pt]-lambda_phi_injected_HX[rap][pt]; 			KinDepRaperr_low[pt]=param_lph_HX_sigma[rap][pt];}
						if(iLam==93){ KinDepRap[pt]= param_ltp_HX_mean[rap][pt]-lambda_thetaphi_injected_HX[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_HX_sigma[rap][pt];}
						if(iLam==94){ KinDepRap[pt]= param_lthstar_HX_mean[rap][pt]-lambda_thetastar_injected_HX[rap][pt]; 	KinDepRaperr_low[pt]=param_lthstar_HX_sigma[rap][pt];}
						if(iLam==95){ KinDepRap[pt]= param_lphstar_HX_mean[rap][pt]-lambda_phistar_injected_HX[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_HX_sigma[rap][pt];}
						if(iLam==96){ KinDepRap[pt]= param_ltilde_HX_mean[rap][pt]-lambda_tilde_injected_HX[rap][pt]; 		KinDepRaperr_low[pt]=param_ltilde_HX_sigma[rap][pt];}

						if(iLam==97){ KinDepRap[pt]= param_lth_PX_mean[rap][pt]-lambda_theta_injected_PX[rap][pt]; 			KinDepRaperr_low[pt]=param_lth_PX_sigma[rap][pt];}
						if(iLam==98){ KinDepRap[pt]= param_lph_PX_mean[rap][pt]-lambda_phi_injected_PX[rap][pt]; 			KinDepRaperr_low[pt]=param_lph_PX_sigma[rap][pt];}
						if(iLam==99){ KinDepRap[pt]= param_ltp_PX_mean[rap][pt]-lambda_thetaphi_injected_PX[rap][pt]; 		KinDepRaperr_low[pt]=param_ltp_PX_sigma[rap][pt];}
						if(iLam==100){ KinDepRap[pt]= param_lthstar_PX_mean[rap][pt]-lambda_thetastar_injected_PX[rap][pt]; KinDepRaperr_low[pt]=param_lthstar_PX_sigma[rap][pt];}
						if(iLam==101){ KinDepRap[pt]= param_lphstar_PX_mean[rap][pt]-lambda_phistar_injected_PX[rap][pt]; 	KinDepRaperr_low[pt]=param_lphstar_PX_sigma[rap][pt];}
						if(iLam==102){ KinDepRap[pt]= param_ltilde_PX_mean[rap][pt]-lambda_tilde_injected_PX[rap][pt]; 		KinDepRaperr_low[pt]=param_ltilde_PX_sigma[rap][pt];}

						if(iLam==103){  KinDepRap[pt]= param_ltilde_CS_mean[rap][pt]; 		 KinDepRaperr_low[pt]=param_ltilde_CS_sigma_low[rap][pt];	 KinDepRaperr_high[pt]=param_ltilde_CS_sigma_high[rap][pt];	 KinDepRap2[pt]= param_ltilde_HX_mean[rap][pt]; 		 KinDepRap2err_low[pt]=param_ltilde_HX_sigma_low[rap][pt];	 KinDepRap2err_high[pt]=param_ltilde_HX_sigma_high[rap][pt];	KinDepRap3[pt]= param_ltilde_PX_mean[rap][pt]; 		 	 KinDepRap3err_low[pt]=param_ltilde_PX_sigma_low[rap][pt];	KinDepRap3err_high[pt]=param_ltilde_PX_sigma_high[rap][pt];}
						if(iLam==104){ KinDepRap[pt]= param_lthstar_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lthstar_CS_sigma_low[rap][pt];	KinDepRaperr_high[pt]=param_lthstar_CS_sigma_high[rap][pt];	KinDepRap2[pt]= param_lthstar_HX_mean[rap][pt]; 		KinDepRap2err_low[pt]=param_lthstar_HX_sigma_low[rap][pt];	KinDepRap2err_high[pt]=param_lthstar_HX_sigma_high[rap][pt];	KinDepRap3[pt]= param_lthstar_PX_mean[rap][pt]; 		KinDepRap3err_low[pt]=param_lthstar_PX_sigma_low[rap][pt];	KinDepRap3err_high[pt]=param_lthstar_PX_sigma_high[rap][pt];}
						if(iLam==105){ KinDepRap[pt]= param_lphstar_CS_mean[rap][pt]; 		KinDepRaperr_low[pt]=param_lphstar_CS_sigma_low[rap][pt];	KinDepRaperr_high[pt]=param_lphstar_CS_sigma_high[rap][pt];	KinDepRap2[pt]= param_lphstar_HX_mean[rap][pt]; 		KinDepRap2err_low[pt]=param_lphstar_HX_sigma_low[rap][pt];	KinDepRap2err_high[pt]=param_lphstar_HX_sigma_high[rap][pt];	KinDepRap3[pt]= param_lphstar_PX_mean[rap][pt]; 		KinDepRap3err_low[pt]=param_lphstar_PX_sigma_low[rap][pt];	KinDepRap3err_high[pt]=param_lphstar_PX_sigma_high[rap][pt];}

						if(emptyBin[rap][pt]){ KinDepRap[pt]=999; KinDepRap2[pt]=999; KinDepRap3[pt]=999;}

						if(iLam<103 && !RealData) {KinDepRaperr_high[pt]=KinDepRaperr_low[pt];}
					...
					if(iLam<103) PlotRapPt(ptBinMin, ptBinMax, rapBin, lammin, lammax, axislabel, filename, KinDepRap, KinDepRaperr_low, KinDepRaperr_high, DrawLine, lamline, GraphFolder, GraphName, SaveGraph, nState, nFrame);
					else PlotComparisonRapPt(ptBinMin, ptBinMax, rapBin, lammin, lammax, axislabel, filename, KinDepRap, KinDepRaperr_low, KinDepRaperr_high, KinDepRap2, KinDepRap2err_low, KinDepRap2err_high, KinDepRap3, KinDepRap3err_low, KinDepRap3err_high, lamline, nState);
		    	...
		    	if(iFrame==3) {sprintf(framerap,"\\hline \\multicolumn{7}{|c|}{PX frame, $%1.1f < |y| < %1.1f$}\\\\ \\hline \\rule{0pt}{4mm}\n",onia::rapForPTRange[rapBin-1],onia::rapForPTRange[rapBin]); fprintf(NumFile,framerap);}
					...
					int pt=0;
						for(int ptBin = ptBinMin; ptBin < ptBinMax+1; ptBin++) {
							...
							if(iTab==1){
								if(iFrame==1){
									lth_tab=param_lth_CS_mean[rap][pt]			;   ltherr_tab=param_lth_CS_sigma_low[rap][pt];            ltherr_high_tab=param_lth_CS_sigma_high[rap][pt];
									lph_tab=param_lph_CS_mean[rap][pt]			;   lpherr_tab=param_lph_CS_sigma_low[rap][pt];            lpherr_high_tab=param_lph_CS_sigma_high[rap][pt];
									ltp_tab=param_ltp_CS_mean[rap][pt]			;   ltperr_tab=param_ltp_CS_sigma_low[rap][pt];            ltperr_high_tab=param_ltp_CS_sigma_high[rap][pt];
									ltilde_tab=param_ltilde_CS_mean[rap][pt]	;   ltildeerr_tab=param_ltilde_CS_sigma_low[rap][pt];      ltildeerr_high_tab=param_ltilde_CS_sigma_high[rap][pt];
									lthstar_tab=param_lthstar_CS_mean[rap][pt]	;   lthstarerr_tab=param_lthstar_CS_sigma_low[rap][pt];    lthstarerr_high_tab=param_lthstar_CS_sigma_high[rap][pt];
									lphstar_tab=param_lphstar_CS_mean[rap][pt]	;   lphstarerr_tab=param_lphstar_CS_sigma_low[rap][pt];    lphstarerr_high_tab=param_lphstar_CS_sigma_high[rap][pt];
								}
								if(iFrame==2){
									lth_tab=param_lth_HX_mean[rap][pt]			;   ltherr_tab=param_lth_HX_sigma_low[rap][pt];            ltherr_high_tab=param_lth_HX_sigma_high[rap][pt];
									lph_tab=param_lph_HX_mean[rap][pt]			;   lpherr_tab=param_lph_HX_sigma_low[rap][pt];            lpherr_high_tab=param_lph_HX_sigma_high[rap][pt];
									ltp_tab=param_ltp_HX_mean[rap][pt]			;   ltperr_tab=param_ltp_HX_sigma_low[rap][pt];            ltperr_high_tab=param_ltp_HX_sigma_high[rap][pt];
									ltilde_tab=param_ltilde_HX_mean[rap][pt]	;   ltildeerr_tab=param_ltilde_HX_sigma_low[rap][pt];      ltildeerr_high_tab=param_ltilde_HX_sigma_high[rap][pt];
									lthstar_tab=param_lthstar_HX_mean[rap][pt]	;   lthstarerr_tab=param_lthstar_HX_sigma_low[rap][pt];    lthstarerr_high_tab=param_lthstar_HX_sigma_high[rap][pt];
									lphstar_tab=param_lphstar_HX_mean[rap][pt]	;   lphstarerr_tab=param_lphstar_HX_sigma_low[rap][pt];    lphstarerr_high_tab=param_lphstar_HX_sigma_high[rap][pt];
								}
								if(iFrame==3){
									lth_tab=param_lth_PX_mean[rap][pt]			;   ltherr_tab=param_lth_PX_sigma_low[rap][pt];            ltherr_high_tab=param_lth_PX_sigma_high[rap][pt];
									lph_tab=param_lph_PX_mean[rap][pt]			;   lpherr_tab=param_lph_PX_sigma_low[rap][pt];            lpherr_high_tab=param_lph_PX_sigma_high[rap][pt];
									ltp_tab=param_ltp_PX_mean[rap][pt]			;   ltperr_tab=param_ltp_PX_sigma_low[rap][pt];            ltperr_high_tab=param_ltp_PX_sigma_high[rap][pt];
									ltilde_tab=param_ltilde_PX_mean[rap][pt]	;   ltildeerr_tab=param_ltilde_PX_sigma_low[rap][pt];      ltildeerr_high_tab=param_ltilde_PX_sigma_high[rap][pt];
									lthstar_tab=param_lthstar_PX_mean[rap][pt]	;   lthstarerr_tab=param_lthstar_PX_sigma_low[rap][pt];    lthstarerr_high_tab=param_lthstar_PX_sigma_high[rap][pt];
									lphstar_tab=param_lphstar_PX_mean[rap][pt]	;   lphstarerr_tab=param_lphstar_PX_sigma_low[rap][pt];    lphstarerr_high_tab=param_lphstar_PX_sigma_high[rap][pt];
								}
							}
