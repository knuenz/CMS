...
//		sprintf(filename,"%s/macros/polFit/MattRes/TGraphResults_%dSUps_MattResults.root",basedir,nState);
//		TFile *MattFileStat = new TFile(filename,"READ");
//		if(nState>1) MattFileStat=infileRes;

//		sprintf(filename,"%s/macros/polFit/MattRes/TGraphResults_%dSUps_MattSyst.root",basedir,nState);
//		TFile *MattFileSyst = new TFile(filename,"READ");
//		if(nState>1) MattFileSyst=infileRes;

		sprintf(filename,"%s/macros/polFit/CDFRes/TGraphResults_%dSUps_CDFStat.root",basedir,nState);
		TFile *MattFileStat = new TFile(filename,"READ");

		sprintf(filename,"%s/macros/polFit/CDFRes/TGraphResults_%dSUps_CDFSyst.root",basedir,nState);
		TFile *MattFileSyst = new TFile(filename,"READ");

		sprintf(filename,"%s/macros/polFit/CDFRes/TGraphResults_%dSUps_CDFTotal.root",basedir,nState);
		...
		sprintf(filename,"/afs/hephy.at/scratch/k/knuenz/CMSSW_4_2_4_patch2/src/UpsilonPol/macros/polFit/Systematics/TotalSyst/%s/TGraphResults_1SUps.root",MPCentralsWithTotalSystID);
		TFile *infileMP1 = new TFile(filename,"READ");
		if(!MultiPanelPlots) infileMP1=infileRes;

		sprintf(filename,"/afs/hephy.at/scratch/k/knuenz/CMSSW_4_2_4_patch2/src/UpsilonPol/macros/polFit/Systematics/TotalSyst/%s/TGraphResults_2SUps.root",MPCentralsWithTotalSystID);
		TFile *infileMP2 = new TFile(filename,"READ");
		if(!MultiPanelPlots) infileMP2=infileRes;

		sprintf(filename,"/afs/hephy.at/scratch/k/knuenz/CMSSW_4_2_4_patch2/src/UpsilonPol/macros/polFit/Systematics/TotalSyst/%s/TGraphResults_3SUps.root",MPCentralsWithTotalSystID);
...
// Declare variables needed for table production
		...
		double val_table[iParameters+1][2][tabPtBins];
		double errHigh_table[iParameters+1][2][tabPtBins];
		double errLow_table[iParameters+1][2][tabPtBins];
		double syst_table[iParameters+1][2][tabPtBins][nSystematics+1];
		double errHighTotal1_table[iParameters+1][2][tabPtBins];
		double errLowTotal1_table[iParameters+1][2][tabPtBins];
		double errHighTotal2_table[iParameters+1][2][tabPtBins];
		double errLowTotal2_table[iParameters+1][2][tabPtBins];
		double errHighTotal3_table[iParameters+1][2][tabPtBins];
		double errLowTotal3_table[iParameters+1][2][tabPtBins];
		double pTmean_table[iParameters+1][2][tabPtBins];
...
//			Ups1SBGratiograph->SaveAs(savequick);
	  ...
	  if(Ups1SBGratioCompZero&&Ups2SBGratioCompZero&&Ups3SBGratioCompZero) sprintf(text,"#Upsilon(1S), #Upsilon(2S) and #Upsilon(3S) mass regions");
	  if(Ups1SBGratioCompZero&&Ups2SBGratioCompZero&&!Ups3SBGratioCompZero) sprintf(text,"#Upsilon(1S) and #Upsilon(2S) mass regions");
	  if(Ups1SBGratioCompZero&&!Ups2SBGratioCompZero&&Ups3SBGratioCompZero) sprintf(text,"#Upsilon(1S) and #Upsilon(3S) mass regions");
	  if(!Ups1SBGratioCompZero&&Ups2SBGratioCompZero&&Ups3SBGratioCompZero) sprintf(text,"#Upsilon(2S) and #Upsilon(3S) mass regions");
	  if(Ups1SBGratioCompZero&&!Ups2SBGratioCompZero&&!Ups3SBGratioCompZero) sprintf(text,"#Upsilon(1S) mass region");
	  if(!Ups1SBGratioCompZero&&Ups2SBGratioCompZero&&!Ups3SBGratioCompZero) sprintf(text,"#Upsilon(2S) mass region");
	  if(!Ups1SBGratioCompZero&&!Ups2SBGratioCompZero&&Ups3SBGratioCompZero) sprintf(text,"#Upsilon(3S) mass region");
...
*/
			...
			if(iLam==3&&rapBin==2){  sprintf(filename,"%s/FinalResultsCS_OLD_Ups%dS.pdf",FigDir,nState);
			if(PlotFinalData) MPcanvasCS->SaveAs(filename);
			MPcanvasCS->Close();
			}
			if(iLam==9&&rapBin==2){  sprintf(filename,"%s/FinalResultsHX_OLD_Ups%dS.pdf",FigDir,nState);
			if(PlotFinalData) MPcanvasHX->SaveAs(filename);
			MPcanvasHX->Close();
			}
			if(iLam==15&&rapBin==2){  sprintf(filename,"%s/FinalResultsPX_OLD_Ups%dS.pdf",FigDir,nState);
...
//   if(iStateMP==3) deltaTrickAxisMax_MPnew=+0.001;
   ...
   char texTexMP[200];
   if(rapBin==1) sprintf(texTexMP,"#Upsilon(%dS)", iStateMP);
...
*/
		    ...
		    for(int rapBin = 1; rapBin < 3; rapBin++) {
		    	...
		    	if(rapBin==1)fprintf(SuppFile,"Table %s, %s, Y(%dS):\n",TabParChar, TabFrameChar,nState);
