...
//// Start variation of PPD ///////////////
		...
		sprintf(filename,"%s/%s_%s/results_%dSUps_rap%d_pT%d.root",storagedir,DefaultID,JobID,nState,rapBin,ptBin);
				...
				for(int iRandPerEntry = 1; iRandPerEntry<nRandPerEntry+1; iRandPerEntry++){
					...
					randomNumber[iRandPerEntry][iLam]=0;
						for(int iSyst = 1; iSyst<nSystematics+1; iSyst++){
							...
							if(ProbDist[iSyst]==2){
								randomNumber[iRandPerEntry][iLam]+= gRandom->Uniform(-SystVariation[iSyst][iFrame][iLam]/2.,  SystVariation[iSyst][iFrame][iLam]/2.);
							}
