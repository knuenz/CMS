...
void PlotComparisonRapPt(int ptBinMin, int ptBinMax, int rapBin, double yMin, double yMax, char yAxisTitle[200], char filename[200], double lmean1[ToyMC::nPtBins], double lmean1err[ToyMC::nPtBins], double lmean2[ToyMC::nPtBins], double lmean2err[ToyMC::nPtBins], double lmean3[ToyMC::nPtBins], double lmean3err[ToyMC::nPtBins]){
...
///////////////////// Extraction of mean injected parameters from GenResults.root //////////////////////////////////////////////////
			  ...
			  double l_min = -1.4;
			  double l_max =  1.4;
			  double l_step_1D = 0.02;

			  double l_min_pull = -5;
			  double l_max_pull =  5;
			  double l_step_1D_pull = 0.5;
...
//////// Rap/Pt Summary Plots ////////////////
	    ...
	    double ddeltamumin=-0.5;
	    double ddeltamumax=0.5;
	    ...
	    for(int rapBin = rapBinMin; rapBin < rapBinMax+1; rapBin++) {
	    	...
	    	if(iLam==103){lammin=ddeltamumin;	lammax=ddeltamumax;	sprintf(filename,"%s/Figures/KinDep_param_mean_Comparison_ltilde_rap%d.pdf",dirstruct,rapBin); 		sprintf(axislabel,"#tilde{#lambda}");}
	    	if(iLam==104){lammin=ddeltamumin;	lammax=ddeltamumax;	sprintf(filename,"%s/Figures/KinDep_param_mean_Comparison_lthstar_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#lambda*_{#theta}");}
	    	if(iLam==105){lammin=ddeltamumin;	lammax=ddeltamumax;	sprintf(filename,"%s/Figures/KinDep_param_mean_Comparison_lphstar_rap%d.pdf",dirstruct,rapBin); 	sprintf(axislabel,"#lambda*_{#phi}");}
					...
					else PlotComparisonRapPt(ptBinMin, ptBinMax, rapBin, lammin, lammax, axislabel, filename, KinDepRap, KinDepRaperr, KinDepRap2, KinDepRap2err, KinDepRap3, KinDepRap3err);
