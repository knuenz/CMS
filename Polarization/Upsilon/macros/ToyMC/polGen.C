...
//const int n_events = 50000; // number of events generated in the entire lepton momentum space

//const double pTdilepton_min = 10.;
//const double pTdilepton_max = 15.;
//const double rapdilepton_min = 0.0; // put here only interval in the positive-rapidity region!
//const double rapdilepton_max = 0.5; // e.g.: [0.5, 1.0] to generate in [-1.0, -0.5] and [0.5, 1.0]


// generated polarization for signal
//const double lambda_theta_sig = 1.0;
...
// background fraction
//const double f_BG = 0.001; // do not set smaller than 0.001

// generated polarization for background
//const double lambda_theta_bkg = -0.5;

// natural frame for background
 bool HX_is_natural_bkg = false;
 bool PX_is_natural_bkg = false;

const double Msignal_peak  =  9.5;
const double Msignal_sigma =  0.1;

// selected invariant mass region
const double M_low =  9.2;
const double M_hgh =  9.8;
...
void polGen(int rapBin = 1,
			int ptBin = 1,
			int n_events = 50000,
			double f_BG = 0.5,
			int polScenSig=1,//see ToyMC.h
			int polScenBkg=1,//see ToyMC.h
	...
	double pTdilepton_min=onia::pTRange[rapBin][ptBin-1];
	double pTdilepton_max=onia::pTRange[rapBin][ptBin];
	double rapdilepton_min=onia::rapForPTRange[rapBin-1];
	double rapdilepton_max=onia::rapForPTRange[rapBin];

	char frameSigName[200];
	sprintf(frameSigName,"%s",onia::frameLabel[frameSig-1]);
	char frameBkgName[200];
	sprintf(frameBkgName,"%s",onia::frameLabel[frameBkg-1]);



	double lambda_theta_sig = ToyMC::ScenarioSig[0][polScenSig-1];
	double lambda_phi_sig = ToyMC::ScenarioSig[1][polScenSig-1];
	double lambda_thetaphi_sig = ToyMC::ScenarioSig[2][polScenSig-1];

	double lambda_theta_bkg = ToyMC::ScenarioBkg[0][polScenBkg-1];
	double lambda_phi_bkg = ToyMC::ScenarioBkg[1][polScenBkg-1];
	double lambda_thetaphi_bkg = ToyMC::ScenarioBkg[2][polScenBkg-1];
	...
	cout<<"Injected lambda_phi Background ............. "<<lambda_phi_bkg<<" , in the "<<frameSigName<<" frame"<<endl;
	cout<<"Injected lambda_thetaphi Background ........ "<<lambda_thetaphi_bkg<<" , in the "<<frameSigName<<" frame"<<endl;
...
/////////////////// CYCLE OF EVENTS ////////////////////////
  for(int i_event = 1; i_event <= n_events; i_event++){
    ...
    if ( gRandom->Uniform() < f_BG ) { mass = gRandom->Uniform(M_low, M_hgh); isBG = 1; }
                                        else { do { mass = gRandom->Gaus(Msignal_peak, Msignal_sigma); }
                                               while ( mass < M_low || mass > M_hgh ); }
