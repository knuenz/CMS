...
void calcLambdastar( double& lthstar, double& lphstar,
  ...
  if ( fabs(lphstarMnus) < fabs(lphstar) ) {
...
void polFit(int n_sampledPoints=1,
  ...
  for ( int i_event = 1; i_event <= n_events; i_event++ ) {
    ...
    double rap  = fabs( dilepton.Rapidity() );
    double mass  = fabs( dilepton.M() );
  ...
  for ( int i_event = 1; i_event <= n_events; i_event++ ) {
    ...
    double  epsilonP = leptonEfficiency->GetBinContent( leptonEfficiency->GetXaxis()->FindBin( fabs( lepP_eta ) ),
                                                        leptonEfficiency->GetYaxis()->FindBin( lepP_pT ) );
    double depsilonP = leptonEfficiency->GetBinError(  leptonEfficiency->GetXaxis()->FindBin( fabs( lepP_eta ) ),
                                                        leptonEfficiency->GetYaxis()->FindBin( lepP_pT ) );
    epsilonP = gRandom->Gaus (epsilonP, depsilonP);

    double  epsilonN = leptonEfficiency->GetBinContent( leptonEfficiency->GetXaxis()->FindBin( fabs( lepN_eta ) ),
                                                        leptonEfficiency->GetYaxis()->FindBin( lepN_pT ) );
    double depsilonN = leptonEfficiency->GetBinError(  leptonEfficiency->GetXaxis()->FindBin( fabs( lepN_eta ) ),
                                                        leptonEfficiency->GetYaxis()->FindBin( lepN_pT ) );
    ...
    if ( epsilon > min_dileptonEff ) {
      ...
      w_sth2c2ph_CS[i_signalEvent] = epsilon * (1. - costh_CS*costh_CS) * cos(gPI/90.*phi_CS);
      w_s2thcph_CS[i_signalEvent]  = epsilon * 2.*costh_CS*sqrt(1.-costh_CS*costh_CS) * cos(gPI/180.*phi_CS);

      w_cth2_HX[i_signalEvent]     = epsilon * costh_HX*costh_HX;
      w_sth2c2ph_HX[i_signalEvent] = epsilon * (1. - costh_HX*costh_HX) * cos(gPI/90.*phi_HX);
      w_s2thcph_HX[i_signalEvent]  = epsilon * 2.*costh_HX*sqrt(1.-costh_HX*costh_HX) * cos(gPI/180.*phi_HX);

      w_cth2_PX[i_signalEvent]     = epsilon * costh_PX*costh_PX;
      w_sth2c2ph_PX[i_signalEvent] = epsilon * (1. - costh_PX*costh_PX) * cos(gPI/90.*phi_PX);
      w_s2thcph_PX[i_signalEvent]  = epsilon * 2.*costh_PX*sqrt(1.-costh_PX*costh_PX) * cos(gPI/180.*phi_PX);
      ...
      rap_effcorr->Fill( fabs( dilepton.Rapidity() ), 1./epsilon );
  ...
  for(int i_MCevent = 1; i_MCevent <= n_MCevents; i_MCevent++){
    ...
    double phi_gen = 2. * gPI * gRandom->Uniform(1.);
    ...
    *lepP = lepton_DILEP_xyz;
    ...
    if ( epsilon > min_dileptonEff ) {
        ...
        sum_sth2c2ph_CS += epsilon * (1.- costh_CS*costh_CS) * cos(gPI/90.*phi_CS);
        sum_s2thcph_CS  += epsilon * 2.*costh_CS*sqrt(1.-costh_CS*costh_CS) * cos(gPI/180.*phi_CS);

        sum_cth2_HX     += epsilon * costh_HX*costh_HX;
        sum_sth2c2ph_HX += epsilon * (1.- costh_HX*costh_HX) * cos(gPI/90.*phi_HX);
        sum_s2thcph_HX  += epsilon * 2.*costh_HX*sqrt(1.-costh_HX*costh_HX) * cos(gPI/180.*phi_HX);

        sum_cth2_PX     += epsilon * costh_PX*costh_PX;
        sum_sth2c2ph_PX += epsilon * (1.- costh_PX*costh_PX) * cos(gPI/90.*phi_PX);
        sum_s2thcph_PX  += epsilon * 2.*costh_PX*sqrt(1.-costh_PX*costh_PX) * cos(gPI/180.*phi_PX);
        ...
        PDF_c2ph_vs_ph_CS->Fill( phi_CS, epsilon * cos(gPI/90.*phi_CS) );
        PDF_1_vs_phth_CS->Fill( phith_CS, epsilon );
        PDF_cphth_vs_phth_CS->Fill( phith_CS, epsilon * cos(gPI/180.*phith_CS) );
        ...
        PDF_c2ph_vs_ph_HX->Fill( phi_HX, epsilon * cos(gPI/90.*phi_HX) );
        PDF_1_vs_phth_HX->Fill( phith_HX, epsilon );
        PDF_cphth_vs_phth_HX->Fill( phith_HX, epsilon * cos(gPI/180.*phith_HX) );
        ...
        PDF_c2ph_vs_ph_PX->Fill( phi_PX, epsilon * cos(gPI/90.*phi_PX) );
        PDF_1_vs_phth_PX->Fill( phith_PX, epsilon );
        PDF_cphth_vs_phth_PX->Fill( phith_PX, epsilon * cos(gPI/180.*phith_PX) );
...
//     cout<<i_sampledPoint<<"delta "<<loglikelihood_CS_difference<<" = "<<loglikelihood_CS_candidate<<" - "<<loglikelihood_CS<<endl;
     ...
     if(  loglikelihood_CS_difference > 0.  ||  log( gRandom->Uniform(1.) ) < loglikelihood_CS_difference  ) {
         ...
         if ( i_sampledPoint > n_burnIn ) {  // reject first n_burnIn extractions in the chain
            ...
            if ( fabs( lph_CS ) > 0.5*( 1 + lth_CS ) || lth_CS*lth_CS + 2.*ltp_CS*ltp_CS > 1
            || fabs( ltp_CS ) > 0.5*( 1 - lph_CS )
     ...
     if(  loglikelihood_HX_difference > 0.  ||  log( gRandom->Uniform(1.) ) < loglikelihood_HX_difference  ) {
         ...
         if ( i_sampledPoint > n_burnIn ) {
            ...
            if ( fabs( lph_HX ) > 0.5*( 1 + lth_HX ) || lth_HX*lth_HX + 2.*ltp_HX*ltp_HX > 1
            || fabs( ltp_HX ) > 0.5*( 1 - lph_HX )
     ...
     if(  loglikelihood_PX_difference > 0.  ||  log( gRandom->Uniform(1.) ) < loglikelihood_PX_difference  ) {
         ...
         if ( i_sampledPoint > n_burnIn ) {
            ...
            if ( fabs( lph_PX ) > 0.5*( 1 + lth_PX ) || lth_PX*lth_PX + 2.*ltp_PX*ltp_PX > 1
            || fabs( ltp_PX ) > 0.5*( 1 - lph_PX )
...
///// Extract numerical values of the lambda parameters from the result TTrees /////
  ...
  TH1D* h_ltilde_CS = new TH1D( "h_ltilde_CS", "", int((l_max-l_min)/l_step_1D), l_min, l_max );
  ...
  TH1D* h_ltilde_HX = new TH1D( "h_ltilde_HX", "", int((l_max-l_min)/l_step_1D), l_min, l_max );
  ...
  TH1D* h_ltilde_PX = new TH1D( "h_ltilde_PX", "", int((l_max-l_min)/l_step_1D), l_min, l_max );
